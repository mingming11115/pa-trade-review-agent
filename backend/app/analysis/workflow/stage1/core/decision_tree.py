...
from app.analysis.workflow.stage1.core.node_ids import (
    STAGE1_FORBIDDEN_GATE_NODES as _FORBIDDEN_FROM_REGISTRY,
    canonicalize_node_id,
    is_order_route_node,
    is_prohibition_node,
    is_stage2_signal_node,
    node_order_key,
)
...
STAGE1_FORBIDDEN_GATE_NODES = _FORBIDDEN_FROM_REGISTRY


def _gate_trace_sort_key(node_id: str) -> tuple[int, str]:
    """Order key for gate_trace (semantic registry rank)."""
    return node_order_key(node_id)


def _node_sort_key(node_id: str) -> tuple[int, str]:
    """Order key for decision_trace (semantic registry rank)."""
    return node_order_key(node_id)
...
def _parse_all_branch_outcomes(text: str) -> dict[str, dict[str, str]]:
    ...
    for chunk in chunks:
        ...
        if not hm:
            continue
        nid = canonicalize_node_id(hm.group(1))
    ...
    for line in text.splitlines():
        ...
        if node_m and current is not None:
            current["nodes"].append(
                {
                    "id": canonicalize_node_id(node_m.group(1)),
...
def normalize_bar_range(item: dict[str, Any], *, default_max_seq: int | None = None) -> dict[str, Any] | None:
    """Return a validated structured range; relative sequence strings are unsupported."""
    raw = item.get("bar_range")
    if raw is None:
        return None
    return BarRange.model_validate(raw).model_dump(mode="json")
...
def format_bar_basis_suffix(item: dict[str, Any]) -> str:
    ...
    if not br:
        return ""
    start = BarRef.model_validate(br["start"])
    end = BarRef.model_validate(br["end"])
    return f"（基于日内第{start.day_index}至第{end.day_index}根判断）"
...
def validate_bar_range_field(item: dict[str, Any], path: str) -> list[str]:
    ...
    if isinstance(raw_range, dict):
        try:
            BarRange.model_validate(raw_range)
        except ValueError as exc:
            errors.append(f"{path}: invalid structured bar_range: {exc}")
        return errors

    errors.append(f"{path}: bar_range must be a structured start/end bar identity")
    return errors
...
def build_stage2_gate_wait_response(stage1_json: dict[str, Any]) -> dict[str, Any]:
    ...
    node_id = str(last.get("node_id", "not_extreme_chaos"))
    ...
    return {
        "decision": {
            ...
            "estimated_win_rate_reasoning": "未进入 trader_equation，无交易胜率估计。",
    ...
    for i, item in enumerate(trace):
        ...
        if nid in STAGE1_FORBIDDEN_GATE_NODES:
            errors.append(
                ...
                "请在阶段二节点 trader_equation（止损止盈确定后）评估"
            )
        ans = item.get("answer")
...
def validate_stage2_trace_consistency(stage2: dict[str, Any]) -> list[str]:
    ...
    if (
        ...
        and outcome == "trade"
        and is_prohibition_node(str(terminal.get("node_id", "") or ""))
    ):
        errors.append(
            "terminal.node_id must not be prohibition_scan for a trade; prohibition_scan is only a prohibition scan, "
    ...
    if "trader_equation_principle" in node_ids:
        errors.append(
            "decision_trace must not include 0.3; use 10.3 after stop/target are set"
        )

    # 10 子节点顺序：10.1 → 10.2 → 10.3
    for a, b in (("stop_defined", "stop_not_excessive"), ("stop_not_excessive", "trader_equation")):
    ...
    idx_103 = _index_of(node_ids, "trader_equation")
    idx_11 = next((i for i, n in enumerate(node_ids) if is_order_route_node(n)), -1)

    if order_type in ("限价单", "突破单", "市价单"):
        if idx_103 < 0:
            errors.append(
                "order_type requires trade: decision_trace must include node trader_equation "
                "(交易者方程) with numeric stop/target assessment"
            )
        else:
            item_103 = trace[idx_103]
            if isinstance(item_103, dict) and item_103.get("answer") != "是":
                errors.append("node trader_equation must be 是 when placing an order")
            if idx_11 >= 0 and idx_11 < idx_103:
                errors.append("11 下单方式 nodes must appear after 10.3 交易者方程")
            # 9 须在 10 之前
            idx_9 = next((i for i, n in enumerate(node_ids) if is_stage2_signal_node(n)), -1)
            idx_101 = _index_of(node_ids, "stop_defined")
            if idx_9 >= 0 and idx_101 >= 0 and idx_9 > idx_101:
                errors.append("9 入场信号 must appear before stop_defined 止损")

    if order_type == "不下单" and outcome in ("wait", "reject"):
        term_nid = str(terminal.get("node_id", ""))
        if idx_103 >= 0 and term_nid not in ("trader_equation", "stop_not_excessive", "stop_defined", "follow_through", "signal_direction_ok"):
            pass  # allow other legitimate early stops
        if idx_103 >= 0 and trace[idx_103].get("answer") == "否" and term_nid != "trader_equation":
            errors.append(
                "when 10.3 answer is 否, terminal.node_id should be 10.3"
            )

        # Detect the "no entry plan → terminal=reject" anti-pattern.
        # When signal_bar_quality answer is 否/等待 (no valid signal bar) or stop_defined answer is 否
        ...
        #
        # Exception: if prohibition_scan is genuinely violated (answer=是 AND reason does NOT
        # contain denial phrases), outcome="reject" is correct because the
        # prohibition explicitly terminated the analysis.  prohibition_scan is a stronger
        # stop reason than signal_bar_quality=否 and takes semantic precedence.
        idx_90 = _index_of(node_ids, "signal_bar_quality")
        idx_101 = _index_of(node_ids, "stop_defined")
        idx_90p = _index_of(node_ids, "planned_limit")
        no_entry_plan = False
        if idx_90 >= 0:
        ...
        if no_entry_plan and outcome == "reject":
            # Check if prohibition_scan genuinely triggered (true violation, not AI misuse)
            _DENIAL = ("未触犯", "未违反", "无触犯", "无违规", "通过扫描", "扫描通过", "无禁止", "未触发")
            sec14_genuinely_violated = any(
                is_prohibition_node(str(item.get("node_id", "")))
            ...
            if not sec14_genuinely_violated:
                errors.append(
                    ...
                    "entry plan to evaluate: signal_bar_quality=否/等待 or stop_defined=否 means no trade "
                    "scheme existed, so nothing can be 'rejected' by the trader equation"
                )

    # 整体章节顺序：已评估节点的 rank 应非递减
    ranks = [_node_sort_key(n)[0] for n in node_ids if canonicalize_node_id(n) != "trader_equation_principle"]
