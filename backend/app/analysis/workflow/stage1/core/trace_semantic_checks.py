...
_REASON_REQUIRED_NODE_IDS = frozenset({"trader_equation"})
...
_ORDER_SECTION_9_REQUIRED = frozenset({"signal_bar_quality", "signal_bar_closed", "signal_direction_ok", "signal_not_overlong", "signal_first_entry", "follow_through", "signal_second_entry", "entry_bar_strong"})
...
def _question_matches_tree(expected: str, question: str, *, node_id: str = "") -> bool:
    ...
    if question.startswith("节点"):
        return True
    if node_id == "channel_direction" and _channel_direction_question_ok(question):
        return True
...
def validate_stage2_order_trace_semantics(stage2: dict[str, Any]) -> list[str]:
    ...
    has_9 = any(is_stage2_signal_node(n) for n in node_ids)
    if not has_9:
        errors.append("placing an order requires decision_trace signal-chain nodes")

    item_90 = next(
        (x for x in trace if isinstance(x, dict) and str(x.get("node_id")) == "signal_bar_quality"),
        None,
    )
    item_90p = next(
        (x for x in trace if isinstance(x, dict) and str(x.get("node_id")) == "planned_limit"),
        None,
    )
    if (
        ...
        errors.append(
            "limit order with signal_bar_quality=否/等待 requires planned_limit=是 (background limit path)"
        )

    for required in ("stop_defined", "stop_not_excessive", "trader_equation"):
        if required not in node_ids:
            errors.append(f"placing an order requires decision_trace node {required}")

    item_103 = next(
        (x for x in trace if isinstance(x, dict) and str(x.get("node_id")) == "trader_equation"),
        None,
    )
