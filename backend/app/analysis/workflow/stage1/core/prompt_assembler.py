...
from app.analysis.workflow.stage1.core.compat import AnalysisRecord, build_decision_stance_guidance, format_epoch_for_display, normalize_stance
from app.analysis.workflow.stage1.core.pattern_routing import (
...
from app.analysis.workflow.stage1.core.market_features import (
...
_KLINE_INDEX_NOTE = (
    "棒身份约定：机器引用必须完整包含 bar_timestamp、timeframe、session、day_index；"
    "CME 按 America/Chicago 17:00 实际开盘重置，US_EQUITY_RTH 按 America/New_York 09:30 重置，均自动处理夏令时。"
)
...
- **仅允许英文**：JSON 字段名、规定枚举值、ISO 时间戳、timeframe 与 session。**除此之外不得使用英文**。
...
_STAGE2_TAIL_REMINDER = (
    ...
    "1. signal_bar_quality=否 时，**必须先写 planned_limit** 评估背景限价；仅当 planned_limit 也=否 且无三价方案时，"
    "terminal.outcome=wait（node_id=planned_limit 或 signal_bar_quality）。\n"
    "   **禁止** signal_bar_quality=否 后直接 wait 而跳过 planned_limit/风险节点。\n"
    "   signal_bar_quality=是（有合格信号棒）或 planned_limit=是（计划型限价）且有三价 → 继续风险评估，不得因缺信号棒直接 wait。\n"
    "   禁止写 reject — 你没有东西可以拒绝（除非 trader_equation 已有三价方案）。\n"
    "2. 你有入场方案（entry/stop/target 三价齐全），但 trader_equation 交易者方程不通过？\n"
    "   → 这才可以写 terminal.outcome=reject，node_id=\"trader_equation\"。\n"
    "3. 你有入场方案且 trader_equation 通过？→ terminal.outcome=trade，node_id 为最终节点。\n"
    ...
    "常见错误速查：signal_bar_quality=否 + stop_defined=否 → outcome=wait（不是 reject！）"
).strip()
...
```json
{
  ...
  "bar_analysis": {
    ...
    "last_closed_bar": {"bar_timestamp":"2026-08-11T20:00:00Z","timeframe":"5m","session":"CME","day_index":36},
    "bar_type": "trend_bull|trend_bear|doji|inside|outside_bull|outside_bear|flat|other",
    "signal_bar": {
      "bar_ref": {"bar_timestamp":"2026-08-11T19:55:00Z","timeframe":"5m","session":"CME","day_index":35},
      "quality": "strong|medium|weak|invalid（bar_ref=null 时必须填 invalid，禁止填 null）",
      "reason": "信号棒质量判断"
    },
  ...
  "bar_by_bar_summary": [
    {
      "bar_ref": {"bar_timestamp":"2026-08-11T20:00:00Z","timeframe":"5m","session":"CME","day_index":36},
  ...
  "gate_trace": [
    {
      "node_id": "cycle_identifiable",
      "question": "是否能识别出当前市场周期？",
      "answer": "是",
      "reason": "K线结构特征清晰，可识别为正常通道",
      "branch": "normal_channel",
      "section": "K线识别",
      "bar_range": {"start":{"bar_timestamp":"2026-08-11T19:05:00Z","timeframe":"5m","session":"CME","day_index":25},"end":{"bar_timestamp":"2026-08-11T20:00:00Z","timeframe":"5m","session":"CME","day_index":36}}
    },
    {
      "node_id": "direction_decidable",
      "question": "近期结构是否呈现明确惯性方向？",
      "answer": "是",
      "reason": "LH+LL 结构清晰",
      "branch": "bearish",
      "section": "方向判断",
      "bar_range": {"start":{"bar_timestamp":"2026-08-11T19:25:00Z","timeframe":"5m","session":"CME","day_index":29},"end":{"bar_timestamp":"2026-08-11T20:00:00Z","timeframe":"5m","session":"CME","day_index":36}}
    }
  ],
  "gate_result": "proceed"
}
...
## 阶段一闸门（二元决策树 阶段一闸门，必须执行）

在输出诊断 JSON 前，按《二元决策.txt》与内置提示文本**依次**评估以下节点，并写入 gate_trace：
**当 gate_result=proceed 时，必须包含节点 cycle_identifiable、not_extreme_chaos、direction_decidable、background_near_term_coherent、momentum_enough 共 5 条（data_sufficiency/program_direction/always_in 由程序判定，AI 不输出）**（每条须填 bar_range；中间节点 reason 可留空，见下）：
闸门层：**data_sufficiency 由程序判定**（数据量已通过前置闸门确认）→ cycle_identifiable 识别周期 → not_extreme_chaos 极端混乱
- **节点 cycle_identifiable**：answer 用 是/否；识别出的周期类型写在 **branch**（如 `broad_channel`、`trading_range`），**禁止** branch 写 `yes`/`no`。
方向层：direction_decidable 惯性方向 → background_near_term_coherent 大时间框架 → **program_direction/always_in 由程序判定，AI 不输出** → momentum_enough 惯性强度（**answer 只能用 是/否/中性**；方向或 AIL/AIS 写在 branch，勿写「多头」「空头」作 answer）

**momentum_enough 重要说明：momentum_enough answer=否/中性 ≠ gate_result=wait。**
- momentum_enough answer=否 或 answer=中性：均只代表惯性不足、不做激进趋势跟踪，**阶段二分析必须继续进入**，gate_result 必须为 **proceed**。
- **禁止**因 momentum_enough 判断惯性不足（否/中性）而将 gate_result 设为 wait——这是最常见的错误。
- gate_result=wait 只在以下情况下才成立：cycle_identifiable 无法识别周期（unknown）、not_extreme_chaos 极端混乱（extreme_tr）。
- **校验硬规则**：当 gate_result=wait 时，gate_trace 最后一个节点的 answer 必须是"否"或"等待"，不得为"中性"。
- momentum_enough 答"否"或"中性"时，gate_result 仍为 proceed，阶段二切换为"等待反弹/回撤到位后的顺势信号"策略，并在 watch_points 中明确触发条件。

**禁止在阶段一评估：**
- **0.3**（交易者方程仅为原则；数值检验在阶段二 **trader_equation**）
- **信号→风险→下单**（入场、风险、下单均属阶段二）

**逐棒摘要硬规则：**
- 必须输出 `bar_by_bar_summary`，**恰好 5 条**（分析窗口≥5根时），覆盖最近 5 根已收盘棒，每条的 `bar_ref` 必须照抄数据表的完整身份。数据不足 5 根则覆盖全部已有棒。
...
- 棒的先后只按 `bar_timestamp` 判断；最新已收盘棒的跟随通常为 pending。`day_index` 仅用于显示，不可脱离 timestamp/timeframe/session 单独充当身份。
...
程序已为 data_sufficiency/program_direction/always_in 填充权威判定，**默认不要输出这些节点**。
仅当你识别到程序规则**未捕捉到**的明确结构性依据时，在顶层 `node_overrides` 数组中提交覆盖：
```json
"node_overrides": [
  {"node_id": "program_direction", "answer": "是", "branch": "bearish", "override_reason": "近3根出现强势看跌反转，斜率窗口未捕捉到该结构突变"}
]
```
约束：data_sufficiency/signal_bar_closed 为锁定节点不可覆盖；安全闸门（trader_equation/prohibition_scan）只能朝更保守方向；program_direction answer/branch 须自洽（bullish/bearish↔是，neutral↔中性）；`answer` **禁止**写多头/空头/bullish/bearish，方向只写在 `branch`；不输出时请勿包含该字段。

**program_direction 覆盖门槛（三项全部满足才允许提交）：**
1. 用完整棒身份指明具体是哪根 K 线、哪个结构特征导致方向突变；
2. 该特征明确超出程序三信号（EMA斜率/收盘重心位移/波段结构枢轴）的计算范围——例如出现程序窗口未捕捉到的突破/假突破/多空角力转换；
3. override_reason 须用具体时间戳、日内开盘序号和价格结构描述，不接受"整体看跌""趋势感觉已变"等模糊表述。

**always_in 覆盖门槛（三项全部满足才允许提交）：**
1. 程序判定的 always_in reason 字段中**已出现** "⚠️ 近N根K线…与全窗口…结论存在背离"预警，或近期 K 线有明确的 EMA 跌破/站上事件（收盘价穿越 EMA20 且后续 K 线未立即修复）；
2. 用数据表中的时间戳和日内开盘序号指出导致短期背离的具体棒；
...
- **gate_result=wait/unknown 的合法触发条件只有两个：cycle_identifiable answer≠是（无法识别周期）或 not_extreme_chaos answer=是（极端混乱 extreme_tr）。** direction_decidable/momentum_enough 答「否」或「中性」、program_direction 中性、方向不明等**均不得**设 gate_result=wait——须 proceed 并在阶段二提高门槛或 wait 下单。
- 区间/信号/风险 等阶段二节点的"否"答案不代表阶段一闸门阻断。
- gate_result=proceed 表示可通过闸门进入阶段二；wait/unknown 表示不应进入策略与下单评估（仅 cycle_identifiable/not_extreme_chaos 可触发）
...
- **中间节点 reason 可省略**：gate_trace / decision_trace 中除下列情况外，`reason` 可留空 `""` 或填 `"—"`，勿在每条分支重复长段说明：
  ...
  - 阶段二 **trader_equation** 交易者方程（须写方程用到的入场/止损/目标数字或胜率假设）
  - node_overrides 的说明写在 **override_reason**，不要求 trace.reason
- bar_range、question、answer 仍必填；程序填充节点（data_sufficiency/program_direction/always_in/signal_bar_closed–follow_through/order路由）由程序写入 reason，AI 不输出
- **禁止在 gate_trace 中输出 node_id 为 "prohibition_scan" 的节点**：prohibition_scan（禁止行为扫描）由程序自动注入，AI 输出会导致重复节点和校验失败
- **禁止在 gate_trace 中输出 `gate_end` / `gate_summary` / `summary` 等汇总节点**：`gate_result` 字段已表达 proceed/wait/unknown；闸门通过说明写在 **最后一条真实节点**（通常是 momentum_enough）的 reason 中，answer 只能用 是/否/中性/等待/不适用
- 节点 always_in / momentum_enough 的 question 须与决策树原文一致（含 Always In 空格、用「支持」而非仅改措辞）

**每条 gate_trace / decision_trace 必须包含 bar_range（K线依据，由你自行判断）：**
- `bar_range` 必须是 `{start: bar_ref, end: bar_ref}`；start 是较早棒、end 是较晚棒，两个端点都完整包含 timestamp/timeframe/session/day_index
- 端点必须逐字照抄当前数据表，不得生成相对 K 编号，不得仅填写 day_index
- **每个节点的 bar_range 应不同**（除非该节点确实与上一节点使用完全相同窗口）；禁止所有节点照抄同一个范围
- **reason 里引用的每根棒**都必须落在该条 `bar_range` 的时间范围内，并使用「日内第 N 根 + 开盘时间 + 周期」表述。
...
- **阶段二 prohibition_scan（禁止行为扫描，仅 decision_trace）**：`answer=是` = **触犯**禁止项；`answer=否` = **未触犯**。禁止用 `是` 表示「扫描完成/通过」。阶段一 gate_trace **不得**输出 prohibition_scan/prohibition_scan。
- **禁止照抄**本提示 JSON 示例里的占位文字或说明中的举例数字；必须对应当前 K 线表与你在 reason 中的分析
- 跳过节点（skipped:true）：answer=不适用，bar_range 填 null。
...
- **<50 且 key_signals 为空**：阶段二强烈倾向 `order_type=不下单`（仍须经 信号→风险 完整评估，不得跳过）
...
- **突破后必须更新**：若最新已收盘棒跌破原支撑或突破原阻力，必须重算对应结构位。增量分析时禁止照抄上一轮 support/resistance。
...
**bar_range 必须使用结构化 start/end 棒身份；不得输出相对 K 编号。**
...
**⚠️ bar_analysis.bar_type 强制规则：必须直接沿用阶段一 `bar_analysis.bar_type` 的值，禁止在阶段二重新推断或修改。** 该值来自程序对最新已收盘棒的 `_classify_bar` 结果；若与肉眼感受不同，可在 reasoning 说明，但字段不得改。

**⚠️ bar_type 单字段优先级（全系统唯一标准，与程序 `_classify_bar` 一致）：**
- 每根 K 线只有**一个** `bar_type`；几何特征表「类型」列、`bar_by_bar_summary[].bar_type`、`bar_analysis.bar_type`（最新已收盘棒）**均须与程序预计算值完全一致**。
- 判定顺序（前一项命中则不再往下）：
...
- `diagnosis_summary.direction` 必须与 `stage1.direction` **保持一致**，除非你在阶段二的 decision_trace 中以 **node_id="program_direction"** 明确记录方向变更及原因。
- **例外（无需 program_direction 节点）**：
  - 阶段一 direction=**neutral** → 阶段二 direction=bullish/bearish：程序判不了方向时 AI 阶段二识别出方向属于正常补充，校验器已豁免。不强制补写 program_direction，但建议补（给本人看更清晰）。
...
- 若阶段一 direction=bullish/bearish，而阶段二判断方向反转，**必须**在 decision_trace 中加入：
  ```json
  {"node_id": "program_direction", "section": "方向重判", "question": "阶段二是否重新判定市场方向？", "answer": "是", "branch": "bullish", "reason": "说明为何方向改变的具体依据", "skipped": false, "bar_range": "由你填写"}
  ```
  做空方向则 `"branch": "bearish"`。**`branch` 字段必须填写且值必须与 `diagnosis_summary.direction` 完全一致**（`bullish` 或 `bearish`）。
- 其他情况若未加 program_direction 节点而 direction 不同，校验器**必定报错**。最稳妥的做法：**让 diagnosis_summary.direction 直接沿用阶段一的 direction 值**，只在有充分依据时才覆盖。

## 阶段二决策路径（二元决策树 结构到下单全路径、prohibition_scan）

阶段一 gate_result=proceed 时，decision_trace 必须遵守**执行顺序**（可跳过不适用分支，但不可乱序）：

1. **结构分支** 按 cycle_position 走对应结构分支（尖峰/通道/区间/反转/楔形等）
2. **信号链执行顺序（硬规则）**：signal_bar_quality（信号棒）→ 若否则 **planned_limit（背景限价，必填）** → signal_first_entry/signal_second_entry/entry_bar_strong（AI）→ signal_bar_closed–follow_through（程序填充）。
   - **signal_bar_quality=否 不是终局**；必须评估 planned_limit 后才能决定 wait。
   - **planned_limit=是** → 继续风险/下单路由 尝试限价单；三价写入 decision，**禁止**只在 watch_points 写触发价。
3. **signal_bar_quality、planned_limit、signal_first_entry、signal_second_entry、entry_bar_strong 由 AI 判定**，须写入 decision_trace
   - **signal_bar_closed/signal_direction_ok/signal_not_overlong/follow_through 由程序填充，AI 不输出**（程序依据几何特征确定性判断）
4. **风险节点** 风险收益（必须按序）：**10.1 止损明确 → 10.2 止损不过大 → trader_equation 交易者方程**（勿编造具体手数、合约数或资金规模）
5. **order路由 下单方式由程序填充，AI 不输出**（程序依据 cycle_position 路由，仅当 trader_equation=是 且下单时填充）
6. **prohibition_scan** 禁止行为清单：下单前快速扫描，触犯任一条 → order_type=不下单
   - **⚠️ prohibition_scan answer 语义硬规则（违反会被程序强制改为不下单）：**
     - `answer=是` = **触犯了禁止行为**（程序据此强制 order_type=不下单）
     - `answer=否` = **未触犯任何禁止项**（可以继续下单）
   - **未触犯时必须写 `answer=否`**，不能写 `是`。许多 AI 误用 `是` 表示"已完成扫描"，这是错误的。
   - 例：扫描完成、无触犯 → `{"node_id":"prohibition_scan","answer":"否","reason":"扫描prohibition_scan：未触犯任何禁止项。①...②..."}`
   - 例：触犯了宽通道追突破 → `{"node_id":"prohibition_scan","answer":"是","reason":"触犯：宽通道中追突破，放弃入场，order_type=不下单"}`

**node_overrides（可选，默认不输出）：**
仅当你识别到程序规则未捕捉到的明确结构性依据时，在顶层 `node_overrides` 数组中提交覆盖（如改变 signal_direction_ok/signal_not_overlong/order 路由）：
```json
"node_overrides": [
  {"node_id": "signal_not_overlong", "answer": "否", "override_reason": "信号棒虽ATR比值略超2，但止损结构合理，程序未考虑此场景"}
]
```
约束：signal_bar_closed 为锁定节点不可覆盖；order 路由可横向切换（限价/突破/市价），但「不下单」不能改为下单；不输出时请勿包含该字段。

**交易者方程（trader_equation）规则：**
- 必须使用 **decision 中已填写的 entry_price / stop_loss_price / take_profit_price** 做数值计算（**take_profit_price_2 不参与 trader_equation**），**禁止**用 K 线收盘、信号棒极点间距或「计划中的 1.8 点/3 点」代替三价
- **突破单须先定 entry 再定 stop/target**：按下方「极值±1跳动」公式写入 `entry_price` 后，再用这三价做 trader_equation；程序校验前会把错误的突破 entry **校正**为极值±跳动。校正后若盈亏比/方程仍不达标，**trader_equation 必须判否**且 `order_type=不下单`
- `decision_trace[trader_equation].reason` 中的入场/止损/目标数字必须与 `decision` 三价一致（勿用未写入 decision 的中间价）
...
- **定价顺序（推荐）**：
  ...
  6. 若结构 stop 已是最宽合理位且 RR 仍 > 1.0：程序会自动扩 stop；只要 trader_equation 交易者方程通过即可
...
- **TP1 / TP2 硬规则**：
  ...
  - trader_equation 交易者方程与 RR 校验**仅使用 take_profit_price（TP1）**；TP2 不得用于方程计算
- 有下单时：盈亏比须 **≥ 1.0**（回报÷风险），且须满足 **胜率%×回报 > (100−胜率)%×风险**
- 不满足上述任一条 → **trader_equation 必须判「否」**，order_type=**不下单**，不得输出限价/突破/市价单
- **trader_equation 通过之前**不得输出具体下单类型；**trader_equation 之后**才写 order路由
- 因方程不通过而放弃（已有三价方案）：terminal.node_id 应为 **trader_equation**，outcome=**reject**（有方案可拒，不用 wait）
- 完成 trader_equation 后，必须把你在方程中使用的**胜率主观估计**写入 decision.estimated_win_rate（0–100 整数），并在 estimated_win_rate_reasoning 简要说明依据；**禁止**留空或仅从 trace 文字里暗示

**结构型止损 / 止盈质量规则（防止噪音内小单）：**
- `stop_loss_price` 必须放在交易假设真正失效的结构位之外，不得贴近 EMA、最新棒 high/low 或单棒噪音。
- 若止损只是在 EMA / 支撑 / 阻力外侧很近的位置，且没有越过明确 swing low/high、信号棒极点、通道边界失效位或区间边界失效位，则视为「噪音内止损」；stop_defined 或 stop_not_excessive 应判「否」。
- `take_profit_price`（TP1）应放在有结构依据的最近有效目标位，不要选择最新棒内部噪音位
...
- 阶段一 `gate_result=proceed` 且 `cycle_position` 为 broad_channel / trading_range / normal_channel / trending_tr 时，**默认先评估计划型限价（planned_limit）**。
- **无独立信号棒（signal_bar.bar_ref=null）** → signal_bar_quality=否，**必须**继续 planned_limit；若 planned_limit=是 则给出限价三价。
- `direction=neutral`、最新棒为 doji/inside/弱棒、或 `transition_risk=medium` 单独出现时，仍应尝试 planned_limit。
- 禁止以「等下一根 K 确认信号棒」为由跳过 planned_limit——计划型限价本来就是等价格到位。
- 仅当区间/通道中部、无结构锚点、最新棒已穿过计划 entry/stop，或铁丝网且无边界锚点时，才可在 planned_limit 判否。

**低质量计划型限价降级规则（已弱化 — 仅作风控提醒，非默认不下单）：**
- 以下情形倾向降低 trade_confidence，但不自动改为不下单：neutral、transition_risk 偏高、diagnosis_confidence<50、最新棒弱且无跟随、方程边际通过。
- 只有 **trader_equation 不通过** 或 **prohibition_scan 触犯** 时，才必须 `order_type=不下单`。

**突破单不可用时的限价单备选路径（重要）：**
- 当通道/趋势结构默认倾向突破单，但**当前没有合格突破入场**（信号棒失效、无跟随、极点不清晰、无法填写 entry_basis_bar/extreme、突破已错过等）时，**不要直接输出「不下单」**。
- 若结构方向仍清晰，且能在**支撑/阻力/通道边界/EMA/前棒极点**附近设定限价 entry，并能给出清晰的**结构失效止损**与**有效结构目标**，且 **trader_equation 交易者方程可通过（数学期望为正）** → **应尝试 `order_type=限价单`**。
- 限价备选典型场景：顺势回撤到结构位做多/做空、区间边界反弹/回落、宽通道靠边界挂单、突破测试失败后的反向结构位。
- 限价单 `entry_basis_*` 可填 null；`signal_bar.bar_ref` 可为 null（quality=invalid），须在 **planned_limit** 说明「计划型限价，等待回撤/反弹到位」；`entry_bar` 设 not_triggered/pending。
- 仅当**突破与限价两种路径均无法**给出满足 stop_defined–trader_equation 的三价方案时，才 `order_type=不下单`。

**planned_limit 计划型限价（宽通道/区间/通道边界 — 无合格信号棒时的正式路径）：**
- **signal_bar_quality** 只评「是否已有合格收盘信号棒」；无信号棒 → signal_bar_quality=否，**必须**写 **planned_limit**。
- 当 cycle_position 为 **broad_channel / trading_range / normal_channel / trending_tr**，且价格靠近 **支撑/阻力/通道边界**（阶段一 support_levels/resistance_levels），或顺势 **回撤/反弹到结构位** 可挂限价时：
  - **planned_limit 应判「是」**，reason 写明「计划型限价，等待回撤/反弹到位」；
  - signal_bar_quality 同时写「否」（无合格信号棒）；
  - `signal_bar.bar_ref` 为 **null**，`quality=invalid` 或 **weak**；
  - `entry_bar` 设 `strength=not_triggered`、`freshness=pending`；
  - 继续 风险节点 定三价 → trader_equation 通过 → `order_type=限价单`。
- **禁止**仅因最新棒为 doji/弱棒/无跟随就直接不下单，应先尝试结构位限价。
- **planned_limit=否/等待**：区间/通道中部、重叠区无边界锚点、无结构锚点定 stop、最新棒已穿过计划 entry/stop、或 prohibition_scan 触犯。
- **direction=neutral 时**：planned_limit 默认 **wait**（禁止双边边界挂单）；仅 signal_bar_quality=是 且有合格顺向信号棒时可下单。
...
**最新已收盘棒新鲜度（计划型 pending 与已触发区分）**：
- **计划型限价**只检查 entry 相对最新已收盘棒 close 的方向；其历史影线触及 entry 不算未来计划已触发，但触及 stop 仍必须不下单。
- **非计划型 / 已触发限价**用最新已收盘棒完整 high/low 对照；close 已在 entry 错误一侧时重新定价或不下单。
...
- 做多突破单：entry_basis_extreme 必须为 "high"，entry_basis_bar 填完整棒身份，`entry_price = high + 1×最小跳动`。
- 做空突破单：entry_basis_extreme 必须为 "low"，entry_basis_bar 填完整棒身份，`entry_price = low − 1×最小跳动`。
- **做空突破单 basis 必须是 low**：即使叙事是「反弹至高点做空」，突破单仍挂在依据 K 的 **低点下方** 突破位；禁止写 `entry_basis_extreme="high"`（与「限价在高点附近做空」不同）。
- entry_rule 只写挂单位置规则（如「依据棒高点上方 1 跳动」），禁止重复 order_type/方向或公式串。
...
**信号链逐K与新鲜度硬规则：**
- signal_bar_quality–entry_bar_strong 必须引用 `signal_bar.bar_ref` 与阶段一摘要的对应身份；计划型限价可为 null。尚未触发时 `entry_bar.bar_ref` 为 null 并设 pending。
- **市价单 entry_bar 硬规则**：`entry_bar.bar_ref` 必须填写当前最新已收盘棒的完整身份，禁止 null/pending。
- 信号棒必须按 `bar_timestamp` 严格早于入场棒；不得用 day_index 大小跨交易时段推断先后。
- 如果信号棒之后已经出现 2–3 根无跟随、反向强 K、或 `entry_bar.freshness=stale|invalid`，不得继续把旧信号当作新的突破单依据。
- 如果最新已收盘棒是 doji、弱入场棒、无跟随或反向确认，应降低 trade_confidence；计划型限价边界仍继续评估，不要仅因最新棒不完美就不下单。
- 当 `bar_analysis.signal_bar.quality=weak|invalid`，或已触发入场棒但 `entry_bar.follow_through=false` 时，若仍下单，必须在信号链节点和 reasoning 中明确说明为何该弱点未使信号失效；否则应等待。挂单未触发时不得把 `follow_through=false` 当作失败跟随，应写 `pending`。
- **计划型限价单**：quality=weak|invalid 且 entry_bar 为 pending 时，**不视为**必须观望；须在 **planned_limit** 判「是」并说明结构位/setup 依据。
...
**跳过规则：**
- 无持仓：跳过 持仓管理节点（不写 trace）
- 不适用分支：skipped:true，answer=不适用

terminal 必须与 order_type 一致（**decision 与 decision_trace 同步**）：
- 有下单 → outcome=trade，trader_equation 必须为「是」，decision 含有效三价
- 不下单 → outcome=wait 或 reject，order_type=不下单，三价与 order_direction 均为 null
- **禁止** decision 写突破单/限价单/市价单，同时 decision_trace 里 trader_equation=否 或 terminal=reject
- **prohibition_scan 是禁止行为扫描，不是成交终局节点**：若 prohibition_scan answer=否（未触犯）且有下单，terminal.node_id 应填最终下单路由 下单节点（如 `order_breakout`/`order_limit`）或 `trader_equation`，**禁止**填 `prohibition_scan`；只有 prohibition_scan answer=是（触犯）时才可作为拒绝/等待的终止原因，且必须 `order_type=不下单`。

**⚠️ terminal.node_id 和 outcome 的语义规则（必须区分以下两种情形）：**

情形 A：**有入场计划，但交易者方程不通过**（有具体止损、止盈数字，但盈亏比不达标）
→ `terminal.node_id = "trader_equation"`，`outcome = "reject"`
→ 典型表现：trader_equation trace 里有具体数值计算，方程结果为负

情形 B：**signal_bar_quality=否 且 planned_limit=否**（或 stop_defined=否 因无止损锚点）
→ `terminal.node_id = "planned_limit"`（或 signal_bar_quality），`outcome = "wait"`
→ **不能** terminal 在 trader_equation，因为从未有过可评估的交易方案
→ **不能** 写 outcome="reject"——拒绝一个不存在的方案在语义上是无意义的

常见错误：signal_bar_quality=否 → **跳过 planned_limit** → stop_defined=否 → terminal=trader_equation/reject
正确做法：signal_bar_quality=否 时**必须先写 planned_limit**；仅当 planned_limit 也=否（或 stop_defined 因无锚点=否）→ terminal.node_id=**planned_limit**（或 signal_bar_quality），outcome=**wait**；trader_equation 不应出现在 trace 里（或标 skipped=true）。**禁止**在未评估 planned_limit 时因 signal_bar_quality=否 直接 terminal=signal_bar_quality。

阶段一 gate_result 为 wait/unknown 时：系统会短路，不应调用本阶段。

**decision.reasoning 字数硬限制：≤280 字（中文）**。只写结论 + 1–2 个关键依据（周期/信号/为何不下单或为何入场），细节放在 decision_trace 的 trader_equation 或 key_factors，勿重复推演整棵决策树。
...
- **必须在 trader_equation 交易者方程评估完成后**由你自行判断并填写；须与 trader_equation 节点 reason 中的胜率假设一致
...
def _stage1_output_reminder_for_mode(analysis_mode: str = "original") -> str:
    ...
    Program prefill handles data_sufficiency/program_direction/always_in; AI outputs the five gate nodes only.
    """
...
COMMON_SYSTEM_STAGE1_TXT_FILES: tuple[str, ...] = (
    "提示词大纲_人设与思维方式.txt",
    "二元决策.txt",           # unified with Stage 2 for prefix caching; 阶段一闸门 gate subset is included
)
...
class PromptAssembler:
    ...
    def _render_kline_table(frame: KlineFrame, limit: int | None = None) -> str:
        ...
        for i, bar in enumerate(bars):
            ...
            lines.append(
                f"{refs[i].bar_timestamp.isoformat()} | {refs[i].timeframe} | {refs[i].session} | {refs[i].day_index} | {bar.open:<9.4f} | {bar.high:<9.4f} | "
                f"{bar.low:<9.4f} | {bar.close:<9.4f} | {yang_yin:<4} | {bar.volume:<9.0f} | "
                f"{ema_str:<10} | {atr_str}"
            )
        lines.append(_KLINE_INDEX_NOTE)
    ...
    def _render_kline_feature_table(frame: KlineFrame, limit: int | None = None) -> str:
        ...
        refs_by_seq = {
            bar.seq: ref for bar, ref in zip(
                frame.bars,
                assign_bar_refs(frame.bars, symbol=frame.symbol, timeframe=frame.timeframe),
            )
        }
        lines = [
            f"（几何特征：最近 {shown} 根已收盘 K 线；「类型」= 单字段 bar_type，优先级 inside/outside > doji/trend/flat/other；多棒形态已用完整窗口计算）",
            "bar_timestamp | timeframe | session | day_index | 类型 | 实体比 | 上影比 | 下影比 | 收盘位置 | Range/ATR | EMA关系 | 与前棒重叠 | ii/iii | ioi | 微双 | 缺口 | EMA缺口数 | 近5突破 | 后续",
            "--------------+-----------+---------+-----------+------+--------+--------+--------+----------+-----------+---------+------------+--------+-----+------+-------+-----------+---------+------",
        ]
        for feat in compute_kline_geometry_features(frame, limit=limit):
            ref = refs_by_seq[feat.seq]
            lines.append(
                f"{ref.bar_timestamp.isoformat()} | {ref.timeframe} | {ref.session} | {ref.day_index} | {feat.bar_type:<13} | "
    ...
    def _render_simple_market_features_block(frame: KlineFrame) -> str:
        ...
        try:
            ...
            return render_simple_market_features(features, frame)
    ...
    def _normalize_prev_stage1_assistant_for_incremental(
        ...
        from app.analysis.workflow.stage1.core.json_validator import format_model_json_for_context
    ...
    def _render_program_prefill_hint(frame: KlineFrame) -> str:
        ...
        exactly what the deterministic engine computed for data_sufficiency / program_direction / always_in
        *before* making its own judgement.  The AI can still override via
        ...
        Why this matters (from prompt_engineering 二元决策.txt program_direction/always_in):
        - program_direction direction is now a 5-signal vote; each signal value is exposed
          so the AI knows which signals contributed and why.
        - always_in Always In now has 3 gates (ratio, slope, swing+pullback); the
        ...
        try:
            ...
            from app.analysis.workflow.stage1.core.trend_context import (
                build_trend_context,
                render_three_window_summary,
            )

            hint_lines: list[str] = [
                "## 程序预填充节点判断依据（data_sufficiency / program_direction / always_in，供 AI 参考）",
                "",
            ...
            hint_lines.append(f"**data_sufficiency 数据是否足够** → {fill_11.answer}")
            hint_lines.append(f"  依据：{build_program_trace_node(fill_11, frame=frame)['reason']}")
            hint_lines.append("")
            ...
            hint_lines.append("")
            hint_lines.append(
                "**background_near_term_coherent 长程背景 vs 近期方向（程序摘要，供 gate_trace background_near_term_coherent 引用）**"
            )
            hint_lines.append(
                f"  长程背景方向（排除最近40根，共{max(0, n_bars_hint - 40)}根）≈ {trend_ctx['background_direction']}；"
            ...
            hint_lines.append(
                f"**program_direction 当前方向（多/空/中性）** → {fill_23.answer}"
                + (f"（branch={fill_23.branch}）" if fill_23.branch else "")
            )
            hint_lines.append(f"  依据：{build_program_trace_node(fill_23, frame=frame)['reason']}")
            hint_lines.append("")

            # always_in
            fill_24 = judge_always_in(frame)
            hint_lines.append(
                f"**always_in 是否 Always In** → {fill_24.answer}"
                + (f"（branch={fill_24.branch}）" if fill_24.branch else "")
            )
            hint_lines.append(f"  依据：{build_program_trace_node(fill_24, frame=frame)['reason']}")
            hint_lines.append("")

            hint_lines.append(
                "⚠️ data_sufficiency 为锁定节点不可覆盖。program_direction/always_in 可通过 node_overrides 覆盖，"
                "但门槛较高：\n"
                "  • program_direction 覆盖须指明具体棒的时间戳、周期、交易时段、日内序号与结构特征；\n"
                "  • always_in 最近8根为主判、最近20根仅作背景参考；覆盖须基于近端结构突变证据；\n"
    ...
    def _build_stage1_user_prompt(self, frame: KlineFrame, *, analysis_mode: str = "original") -> str:
        ...
        if n_bars > 40:
            bg_window = f"**长程背景**（排除最近 40 根后的较老 {n_bars - 40} 根）：\n"
        else:
            bg_window = (
                f"**长程背景**（当前仅 {n_bars} 根，不足 41 根，与近期窗口重叠；"
                f"以程序预填 background_near_term_coherent 为准）：\n"
            )
        return (
            "## 阶段一任务\n\n"
            ...
            "（机器身份=bar_timestamp+timeframe+session；day_index 从实际交易时段开盘起编号；"
            "每个决策节点的 bar_range 使用完整 start/end 棒身份）\n\n"
            f"## ⚠️ 分析窗口分层规则（与程序 background_near_term_coherent/program_direction/always_in 预填一致，必须遵守）\n\n"
            f"你收到全部 {n_bars} 根 K 线数据；下列分层与 `市场诊断框架.txt`、程序三窗口摘要**同一标准**：\n\n"
            f"{bg_window}"
            f"- swing 高低点、磁力位参考 → 写入 `htf_context`；background_near_term_coherent 背景方向\n"
            f"- **禁止**用背景方向否决近期 `direction`；冲突时近期为主、背景作风险参考\n\n"
            f"**近期结构（最近 {min(40, n_bars)} 根）：**\n"
            f"- `cycle_position`、`direction`、通道/区间/波段主结构\n"
            f"- 程序 program_direction 方向投票与多数闸门 bar_range 优先此窗口\n\n"
            f"**即时惯性（最近 {min(8, n_bars)} 根）：**\n"
            f"- always_in Always In、momentum_enough 惯性强度、近端 spike_stage / 尖峰识别\n\n"
            f"**即时信号（最近 {min(10, n_bars)} 根）：**\n"
            f"- 信号棒/入场棒/二次入场/突破失败（阶段二信号链裁定窗口）\n\n"
            f"**逐棒摘要（最近 {min(5, n_bars)} 根）：**\n"
            f"- `bar_by_bar_summary` **必须**恰好 5 条（窗口≥5 根时），每条 1 句 reason\n\n"
            f"## K线数据（时间戳+周期+交易时段为身份；day_index=实际开盘序号；不含当前未收盘K线；"
    ...
    def _build_incremental_stage1_user_prompt(
        ...
        *,
        ...
        return (
            ...
            "- `incremental_delta.new_closed_bars` 必须列出每根新增棒的完整 bar_ref，长度等于新增已收盘棒数量。\n"
            "- 并在 summary / risk_warning / gate_trace 中说明相对上一轮变化。\n"
            "- gate_result=proceed 时 gate_trace 仍须覆盖 cycle_identifiable、not_extreme_chaos、direction_decidable、background_near_term_coherent、momentum_enough（data_sufficiency/program_direction/always_in 由程序填充）。\n"
            ...
            "（身份=bar_timestamp+timeframe+session；day_index=实际交易时段开盘序号；bar_range 使用结构化 start/end）\n\n"
            "## 上一轮已完成分析（仅作为延续上下文）\n\n"
            f"```json\n{json.dumps(previous_summary, ensure_ascii=False, indent=2)}\n```\n\n"
            f"## 新增 K线数据（共{new_count}根，含完整棒身份与阳阴列）\n\n"
            f"{new_kline_table}\n\n"
    ...
    def _build_incremental_stage1_continuation_user_prompt(
        ...
        *,
        ...
        Injects program prefill hint so the AI knows updated program_direction/always_in verdicts
        ...
        return (
            ...
            "完整 K 线数据已包含在上方阶段一用户消息中，每根棒使用稳定身份，你可以回溯查看任何历史 K 线。\n\n"
            "⚠ 反锚定要求——这是增量分析最重要的原则：\n"
            ...
            "- 若最新已收盘棒已突破上一轮 resistance_levels 或跌破 support_levels，必须重算支撑/阻力，"
    ...
    def _build_stage2_user_prompt(
        ...
        *,
        ...
        from app.analysis.workflow.stage1.core.decision_continuity import (
        ...
        from app.analysis.workflow.stage1.core.price_tick import format_breakout_tick_hint
        ...
        return (
            ...
            f"请根据以上诊断和K线数据,按《二元决策.txt》结构到下单全路径、prohibition_scan 输出 JSON 决策结果"
    ...
    def _render_transition_guidance(stage1_json: dict) -> str:
        ...
        return (
            ...
            f"- 入场选择：{selectivity}。\n"
            "- 不因为状态转换而跳过信号/风险/prohibition_scan；只是提高信号质量门槛并降低交易频率。"
        )
    ...
    def _render_planned_limit_hint(stage1_json: dict, frame: KlineFrame) -> str:
        ...
        lines = [
            "## signal_bar_quality / planned_limit 计划型限价提示（程序根据阶段一结构生成）",
            "",
            "**优先级：市场周期 + 方向背景 > 独立信号棒。**",
            f"- cycle_position=**{cycle}** → 默认优先考虑 **限价单**（order 路由），"
            "尤其在通道/区间 **边界** 或 **顺势回撤/反弹结构位**（非中部）。",
            "- 若无强信号棒：signal_bar_quality=否，**必须** 继续写 **planned_limit** 并尝试背景限价三价。",
            "- planned_limit=是：signal_bar.bar_ref=null、quality=invalid；entry_bar pending；"
        ...
        if direction == "neutral":
            lines.append(
                "- 阶段一 direction=neutral：**planned_limit 默认 wait**（禁止双边边界挂单）。"
            )
        return "\n".join(lines) + "\n"
