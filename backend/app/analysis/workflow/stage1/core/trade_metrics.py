...
def _latest_closed_bar(kline_frame: Any) -> Any | None:
    """Return latest closed bar (newest closed bar) from a snapshot frame."""
...
def validate_limit_order_k1_freshness(
    ...
    *,
    ...
    """Reject stale limit orders that latest closed bar has already traded through."""
    ...
    if long is True:
        if pending_planned:
            ...
            if k_close < entry - tick:
                errors.append(
                    f"limit long (planned): latest closed bar close {k_close:.6g} is below entry {entry:.6g}; "
                    "reprice entry or 不下单"
                )
        else:
            if k_low <= entry + tick:
                errors.append(
                    f"limit long: latest closed bar low {k_low:.6g} already touched/below entry {entry:.6g}; "
                    "pending buy limit is stale — use 市价单, reprice, or 不下单"
                )
            if k_close < entry - tick:
                errors.append(
                    f"limit long: latest closed bar close {k_close:.6g} is below entry {entry:.6g}; "
                    "do not keep a buy limit above market without repricing"
                )
        if k_low <= sl + tick:
            errors.append(
                f"limit long: latest closed bar low {k_low:.6g} already at/below stop {sl:.6g}; "
        ...
        if pending_planned:
            if k_close > entry + tick:
                errors.append(
                    f"limit short (planned): latest closed bar close {k_close:.6g} is above entry {entry:.6g}; "
                    "reprice entry or 不下单"
                )
        else:
            if k_high >= entry - tick:
                errors.append(
                    f"limit short: latest closed bar high {k_high:.6g} already reached/exceeded entry {entry:.6g}; "
                    "pending sell limit is stale — use 市价单, reprice, or 不下单"
                )
            if k_close > entry + tick:
                errors.append(
                    f"limit short: latest closed bar close {k_close:.6g} is above entry {entry:.6g}; "
                    "do not keep a sell limit below market without repricing"
                )
        if k_high >= sl - tick:
            errors.append(
                f"limit short: latest closed bar high {k_high:.6g} already at/above stop {sl:.6g}; "
                "plan invalid — order_type=不下单"
            )
    ...
    *,
    ...
    if win_rate is None:
        ...
        errors.append(
            ...
            "trader_equation must be 否 and order_type=不下单 unless prices are fixed"
        )
