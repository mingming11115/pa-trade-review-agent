...
from app.analysis.workflow.stage1.core.decision_tree import normalize_bar_range, validate_bar_range_field
from app.analysis.workflow.stage1.core.node_ids import is_stage2_signal_node

# Target bar count for bar_by_bar_summary when the analysis window has enough bars.
BAR_BY_BAR_TARGET_COUNT = 5

# Stage-one gate nodes required when gate_result=proceed (prompt 阶段一闸门).
...
_OVERRIDE_TRACE_NODES = frozenset({"cycle_identifiable", "program_direction"})
...
def validate_trace_bars_in_frame(
    ...
    *,
    ...
    errors: list[str] = []
    from app.analysis.workflow.stage1.core.bar_identity import BarRange, resolve_bar_ref
    for i, item in enumerate(trace or []):
        ...
        if isinstance(raw_range, dict):
            try:
                structured = BarRange.model_validate(raw_range)
                resolve_bar_ref(kline_frame, structured.start)
                resolve_bar_ref(kline_frame, structured.end)
            except ValueError as exc:
                errors.append(f"{path_prefix}[{i}]: {exc}")
            continue
        if raw_range is not None:
            errors.append(f"{path_prefix}[{i}]: legacy bar_range strings are unsupported")
    return errors
...
def validate_duplicate_bar_ranges(
    ...
    *,
    ...
    for item in trace or []:
        ...
        raw_range = item.get("bar_range")
        if isinstance(raw_range, dict):
            start = raw_range.get("start", {}).get("bar_timestamp")
            end = raw_range.get("end", {}).get("bar_timestamp")
            if start and end:
    ...
    *,
    ...
    for item in summary:
        if not isinstance(item, dict):
            continue
        try:
            bar = resolve_bar_ref(kline_frame, BarRef.model_validate(item.get("bar_ref")))
        ...
        if declared and computed and (declared, computed) in _opposites:
            ...
            corrections.append(
                f"auto-fixed bar_by_bar_summary bar_type: {declared!r} → {computed!r}"
            )
    return corrections
...
def validate_stage1_coherence(
    ...
    *,
    ...
    item_12 = _find_trace_item(gate_trace, "cycle_identifiable")
    if item_12 and not item_12.get("skipped"):
        ...
        if branch_cycle and cycle and branch_cycle not in (cycle, alt_norm):
            errors.append(
                f"gate_trace node cycle_identifiable branch {branch_cycle!r} conflicts with "
                f"cycle_position={cycle!r}"
            )

    direction = str(stage1.get("direction", "") or "").strip().lower()
    item_23 = _find_trace_item(gate_trace, "program_direction")
    if item_23 and not item_23.get("skipped"):
        ...
        if branch_dir and direction and branch_dir != direction:
            if not (cycle in _RANGE_CYCLES and branch_dir == "neutral"):
                errors.append(
                    f"gate_trace node program_direction branch {branch_dir!r} conflicts with "
                    f"direction={direction!r}"
                )
        if ans == "中性" and direction not in ("neutral", ""):
            if cycle not in _RANGE_CYCLES:
                errors.append(
                    "gate_trace node program_direction answer=中性 but direction is not neutral"
                )
        ...
        if n_bars is not None and n_bars > 0:
            ...
            if count != expected:
                errors.append(
                    ...
                    f"(latest {target} closed bars when enough data exists)"
                )
        elif count < 1:
    ...
    from app.analysis.workflow.stage1.core.pattern_routing import validate_detected_patterns_vs_key_signals
    ...
    *,
    ...
    for i, item in enumerate(summary):
        ...
        try:
            ...
            errors.append(f"bar_by_bar_summary[{i}].bar_ref not in geometry feature table")
            continue
        seq = int(bar.seq)
        feat = features.get(seq)
        if feat is None:
            errors.append(
                f"bar_by_bar_summary[{i}].bar_ref not in geometry feature table"
            )
            continue
        declared = str(item.get("bar_type", "") or "").strip().lower()
        computed = str(feat.bar_type or "").strip().lower()
        ...
        if declared in _ALL_BAR_TYPES and computed in _ALL_BAR_TYPES:
            ...
            if (declared, computed) in _opposites or (computed, declared) in _opposites:
                errors.append(
                    ...
                    f"program feature bar_type={computed!r}"
                )
            continue
    return errors
...
def _stage2_trace_documents_override(
    ...
    *,
    ...
    for item in decision_trace:
        ...
        if field == "cycle_position" and nid == "cycle_identifiable":
            if branch == new_value or new_value in reason.lower():
                return True
        if field == "direction" and nid == "program_direction":
            if branch == new_value or new_value in reason.lower():
                return True
    return False
            ...
            if not (s1_cycle in s2_cycle or s2_cycle in s1_cycle):
                if not _stage2_trace_documents_override(
                    ...
                    errors.append(
                        ...
                        f"stage1 {s1_cycle!r}; decision_trace must include node cycle_identifiable "
            ...
            if not (s2_dir == "neutral" and s2_cycle in _RANGE_CYCLES):
                ...
                if not (s1_dir == "neutral"):
                    if not _stage2_trace_documents_override(
                        ...
                        # Auto-inject a minimal node program_direction so the analysis isn't
                        ...
                        if isinstance(trace, list):
                            ...
                            logger.info(
                                "validate_stage2_coherence: auto-injected node program_direction "
                            ...
                            errors.append(
                                ...
                                f"stage1 {s1_dir!r}; decision_trace must include node program_direction "
                                "documenting the change"
                            )
    ...
    if isinstance(decision, dict) and not is_auto_stub:
        ...
        if order_type in ("限价单", "突破单", "市价单") and order_dir in ("做多", "做空"):
            ...
            if not _has_2_3_override:
                ...
                if conflicts:
                    # Auto-inject node program_direction instead of hard-failing.
                    trace = stage2.get("decision_trace")
                    if isinstance(trace, list):
                        # Only inject if not already present from the direction check above
                        already = any(
                            isinstance(x, dict) and str(x.get("node_id", "")) == "program_direction"
                            for x in trace
                        )
                        if not already:
                            auto_23 = {
...
def _validate_stage2_section9(
    ...
    has_section_9 = any(is_stage2_signal_node(n) for n in node_ids)
    ...
    idx_101 = next((i for i, n in enumerate(node_ids) if n == "stop_defined"), -1)
    if idx_9 >= 0 and idx_101 >= 0 and idx_9 > idx_101:
        errors.append("9 入场信号 nodes must appear before stop_defined 止损")
