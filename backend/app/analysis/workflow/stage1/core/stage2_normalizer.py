...
from app.analysis.workflow.stage1.core.node_ids import is_order_route_node, is_prohibition_node

import copy
import logging
from typing import Any

from app.analysis.workflow.stage1.core.trace_normalize import normalize_stage2_traces
from app.analysis.workflow.stage1.core.price_tick import (
    normalize_breakout_basis_extreme,
    normalize_breakout_entry_price,
)
from app.analysis.workflow.stage1.core.bar_identity import BarRef, assign_bar_refs
...
def _section14_violated(trace: Any) -> bool:
    """Return True only when prohibition_scan answer is 是 AND the reason text confirms a violation.

    Background: prohibition_scan question is "是否触犯禁止行为清单？"
...
def _coerce_decision_no_order(out: dict[str, Any]) -> bool:
    ...
    triggers: list[str] = []
    if _trace_node_answer(trace, "trader_equation") == "否":
        triggers.append("trader_equation=否")
    if outcome in ("wait", "reject"):
        triggers.append(f"terminal.outcome={outcome}")
    if _section14_violated(trace):
        triggers.append("prohibition_scan触犯")
...
def _repair_terminal_trade_node(out: dict[str, Any]) -> bool:
    """A successful trade should not terminate at prohibition_scan (prohibition scan)."""
    decision = out.get("decision")
    terminal = out.get("terminal")
    trace = out.get("decision_trace")
    ...
    if not is_prohibition_node(node_id):
        return False
    ...
    if isinstance(trace, list):
        ...
        if replacement is None:
            for item in reversed(trace):
                if not isinstance(item, dict):
                    continue
                if str(item.get("node_id", "") or "").strip() == "trader_equation":
                    replacement = "trader_equation"
                    break

    if replacement is None:
        replacement = "trader_equation"
    terminal["node_id"] = replacement
    logger.debug("terminal.node_id %r -> %r (trade cannot terminate at prohibition_scan)", node_id, replacement)
    return True
...
def _normalize_market_order_entry_bar(
    ...
    if not isinstance(entry_bar, dict) or not isinstance(signal_bar, dict):
        return False
    if entry_bar.get("bar_ref") is not None:
        return False
    if not signal_bar.get("bar_ref") or kline_frame is None or not kline_frame.bars:
        return False
    refs = assign_bar_refs(kline_frame.bars, symbol=kline_frame.symbol, timeframe=kline_frame.timeframe)
    latest_index = min(range(len(kline_frame.bars)), key=lambda i: kline_frame.bars[i].seq)
    entry_bar["bar_ref"] = refs[latest_index].model_dump(mode="json")
    ...
    logger.debug("market order: entry_bar.bar_ref set to latest closed bar")
    return True


def _normalize_signal_entry_bar_chain(
    bar_analysis: dict[str, Any], decision: dict[str, Any], kline_frame: Any
) -> bool:
    """Signal bar must be strictly older than entry bar; pending entry is exempt."""
    if decision.get("order_type") not in _TRADE_ORDER_TYPES:
        return False
    signal_bar = bar_analysis.get("signal_bar")
    entry_bar = bar_analysis.get("entry_bar")
    ...
    pending = (
        ...
        or not entry_bar.get("bar_ref")
        or freshness in ("pending", "stale", "invalid")
    )
    if pending:
        entry_bar["bar_ref"] = None
    ...
    if kline_frame is None:
        return False
    try:
        signal_ref = BarRef.model_validate(signal_bar.get("bar_ref"))
        entry_ref = BarRef.model_validate(entry_bar.get("bar_ref"))
    except (TypeError, ValueError):
        return False
    if signal_ref.bar_timestamp < entry_ref.bar_timestamp:
        return False
    refs = sorted(
        assign_bar_refs(kline_frame.bars, symbol=kline_frame.symbol, timeframe=kline_frame.timeframe),
        key=lambda ref: ref.bar_timestamp,
    )
    older = [ref for ref in refs if ref.bar_timestamp < entry_ref.bar_timestamp]
    if not older:
        signal_bar["bar_ref"] = None
        signal_bar["quality"] = "invalid"
        return True
    signal_bar["bar_ref"] = older[-1].model_dump(mode="json")
    logger.debug("signal_bar.bar_ref moved to preceding closed bar")
    return True
...
def _coerce_decision_when_trade_metrics_fail(
    ...
    *,
    ...
    from app.analysis.workflow.stage1.core.trade_metrics import validate_order_trade_metrics
    ...
    if isinstance(terminal, dict):
        ...
        terminal["node_id"] = "trader_equation"
        terminal.setdefault(
            "label",
            "交易者方程/盈亏比未达标，不下单",
        )
...
def _normalize_next_cycle_prediction(
    ...
    *,
    ...
    from app.analysis.workflow.stage1.core.cycle_enums import CYCLE_ORDER
...
def _fix_background_limit_trace(out: dict[str, Any], kline_frame: Any = None) -> bool:
    """Ensure planned_limit=是 when a planned limit order follows signal_bar_quality=否."""
    try:
        from app.analysis.workflow.stage1.core.decision_nodes import is_planned_limit_order
    except ImportError:
        return False
    ...
    for item in trace:
        if not isinstance(item, dict):
            continue
        nid = str(item.get("node_id", "")).strip()
        if nid == "signal_bar_quality":
            node_90 = item
        elif nid == "planned_limit":
            node_90p = item
    ...
    if node_90 is not None:
        ...
        if ans in ("否", "等待"):
            if node_90p is None:
                bar_range = None
                if kline_frame is not None and kline_frame.bars:
                    from app.analysis.workflow.stage1.core.bar_identity import BarRange, assign_bar_refs
                    selected = list(kline_frame.bars[:10])
                    refs = assign_bar_refs(selected, symbol=kline_frame.symbol, timeframe=kline_frame.timeframe)
                    bar_range = BarRange(start=refs[-1], end=refs[0]).model_dump(mode="json")
                trace.insert(
                    trace.index(node_90) + 1,
                    {
                        "node_id": "planned_limit",
                        "section": "入场信号",
                        "question": "背景驱动限价单评估（signal_bar_quality=否 时必须评估）",
                        "answer": "是",
                        "reason": (
...
def _fix_9_0_for_planned_limit(out: dict[str, Any]) -> bool:
    """When model outputs a valid planned limit but signal_bar_quality=否, upgrade to 是."""
    try:
        from app.analysis.workflow.stage1.core.decision_nodes import is_planned_limit_order
    except ImportError:
        return False
    ...
    for item in trace:
        if not isinstance(item, dict):
            continue
        if str(item.get("node_id", "")).strip() != "signal_bar_quality":
            continue
...
def normalize_stage2(
    ...
    *,
    ...
    if isinstance(decision, dict):
        from app.analysis.workflow.stage1.core.trade_metrics import adjust_decision_stop_for_tp1_rr_cap
    ...
    if _fix_background_limit_trace(out, kline_frame):
        logger.debug("Ensured planned_limit for background planned limit order")
    if _fix_9_0_for_planned_limit(out):
        logger.debug("Upgraded signal_bar_quality to 是 for planned limit order")

    # ── DecisionNodeEngine: fill signal_bar_closed/signal_direction_ok/signal_not_overlong/follow_through/11 ─────────────────────
    if kline_frame is not None:
        try:
            from app.analysis.workflow.stage1.core.decision_nodes import DecisionNodeEngine
    ...
    if isinstance(bar_analysis, dict) and isinstance(decision, dict):
        _normalize_market_order_entry_bar(bar_analysis, decision, kline_frame)
        if _normalize_signal_entry_bar_chain(bar_analysis, decision, kline_frame):
            pass
    if isinstance(bar_analysis, dict):
        signal_bar = bar_analysis.get("signal_bar")
        if isinstance(signal_bar, dict):
            if not signal_bar.get("bar_ref"):
                signal_bar["bar_ref"] = None
        ...
        if isinstance(entry_bar, dict):
            strength = str(entry_bar.get("strength", "") or "").strip().lower()
            has_bar = bool(entry_bar.get("bar_ref"))
            if strength == "not_triggered" or not has_bar:
                ...
                entry_bar["strength"] = "not_triggered"
                entry_bar.setdefault("bar_ref", None)
