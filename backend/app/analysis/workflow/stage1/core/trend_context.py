...
def render_three_window_summary(frame: Any, trend_ctx: dict[str, Any]) -> str:
    ...
    lines = [
        "## 程序三窗口结构摘要（Brooks 单周期多尺度，供 background_near_term_coherent 参考）",
        "",
        f"- **长程背景（排除最近40根后共{max(0, bg_hi - bg_lo + 1)}根）**：主要趋势方向 ≈ **{bg}**（磁力位/阻力支撑参考，不否决近期）",
        f"- **近期结构（最近{min(RECENT_STRUCT_BARS, n)}根）**：交易主方向 ≈ **{td}**（cycle_position / 入场逻辑优先此窗口）",
        f"- **即时惯性（最近{min(SPIKE_NEAR_WINDOW, n)}根）**："
