...
from app.analysis.workflow.stage1.core.coherence_checks import BAR_BY_BAR_TARGET_COUNT
from app.analysis.workflow.stage1.core.trace_normalize import normalize_stage1_traces
...
def _bar_type_from_summary(out: dict[str, Any], bar_ref: object) -> str | None:
...
def _normalize_bar_types(out: dict[str, Any]) -> None:
    ...
    if norm_bt is None:
        last_bar = bar_analysis.get("last_closed_bar")
...
def _normalize_signal_bar_object(out: dict[str, Any]) -> bool:
    ...
    if isinstance(signal_bar, dict):
        ...
        if q is None or not isinstance(q, str) or not str(q).strip():
            ...
            signal_bar["quality"] = "invalid" if signal_bar.get("bar_ref") is None else "weak"
        return False

    inferred = _infer_signal_bar_from_summary(out.get("bar_by_bar_summary"))
    if inferred is None:
        last_bar = bar_analysis.get("last_closed_bar") or "最新已收盘棒"
...
def _summary_bar_seq(bar_ref: object, kline_frame: Any) -> int | None:
    from app.analysis.workflow.stage1.core.bar_identity import BarRef, resolve_bar_ref
    try:
        return int(resolve_bar_ref(kline_frame, BarRef.model_validate(bar_ref)).seq)
...
def _pad_bar_by_bar_summary_to_minimum(
    ...
    *,
    ...
    present = {
        ...
        for seq in (_summary_bar_seq(item.get("bar_ref"), kline_frame),)
        if seq is not None
    }
    ...
    for seq in missing:
        ...
        padded.append(
            {
                "bar_ref": refs_by_seq[seq].model_dump(mode="json"),
                "role": "structure",
                "bar_type": bar_type,
                ...
                "trapped_side": "none",
                "reason": (
                    f"程序补全日内第{refs_by_seq[seq].day_index}根（模型仅写了{len(summary)}条摘要，窗口需至少{expected_min}根）；"
    ...
    merged.sort(key=lambda x: _summary_bar_seq(x.get("bar_ref"), kline_frame) or 0, reverse=True)
...
def _fill_incremental_delta(
    ...
    *,
    ...
    expected_bars = [ref.model_dump(mode="json") for ref in refs[:new_bar_count]]
...
def normalize_stage1(
    ...
    *,
    ...
    # ── DecisionNodeEngine: fill data_sufficiency/program_direction/always_in (before strategy_files routing) ──
    ...
    from app.analysis.workflow.stage1.core.pattern_routing import ensure_detected_patterns_coherent

    if kline_frame is not None:
        try:
            from app.analysis.workflow.stage1.core.market_features import build_program_features_dict
    ...
    if kline_frame is not None:
        try:
            from app.analysis.workflow.stage1.core.structure_levels import refresh_stage1_support_resistance
