...
from app.analysis.workflow.stage1.core.price_tick import infer_price_tick_from_frame
...
def extract_always_in_branch(stage1_json: dict | None) -> str | None:
    """Return AIL / AIS from gate_trace always_in, or None if not Always In."""
    if not stage1_json:
        return None
    for node in stage1_json.get("gate_trace") or []:
        ...
        if nid not in ("always_in", "always_in"):
            continue
...
def assess_plan_invalidation(
    ...
    """True if latest closed bar (latest closed bar) invalidated the previous plan (stop touched)."""
    ...
    if sign < 0:
        if close >= stop or high >= stop:
            return True, f"latest closed bar 触及/突破止损 {stop}（做空方案失效）"
    elif sign > 0:
        if close <= stop or low <= stop:
            return True, f"latest closed bar 触及/跌破止损 {stop}（做多方案失效）"
    return False, ""
    ...
    *,
    ...
    for bar in scan:
        ...
        if sign > 0 and low <= entry + tol:
            return (
                True,
                f"bar low={low} 已触及/跌破限价入场 {entry}（做多限价视为已触发）",
                seq,
            )
        if sign < 0 and high >= entry - tol:
            return (
                True,
                f"bar high={high} 已触及/突破限价入场 {entry}（做空限价视为已触发）",
                seq,
            )
    return False, "", None
...
def render_continuity_prompt_block(ctx: dict[str, Any]) -> str:
    if not ctx.get("has_previous_plan"):
        ...
        if always_in == "AIL":
            neutral_lines.append(
                "- 当前 always_in=**AIL** → **仅允许做多侧** setup（回踩支撑/下边界/顺势回撤做多）。"
                "禁止 planned_limit 阻力做空。"
            )
        elif always_in == "AIS":
            neutral_lines.append(
                "- 当前 always_in=**AIS** → **仅允许做空侧** setup（反弹阻力/上边界/顺势回撤做空）。"
                "禁止 planned_limit 支撑做多。"
            )
        else:
            neutral_lines.append(
                "- 当前 always_in **非** Always In → planned_limit 计划型限价默认 **wait**；"
                "仅当出现与 always_in 方向一致的强信号棒（signal_bar_quality=是）才可下单。"
            )
        neutral_lines.extend([
            ...
            "除非 latest closed bar **收盘**突破上一轮 `invalidation_condition` / 止损结构位。",
            "",
    ...
    lines = [
        ...
        "**禁止**立即在相近结构位反手，除非 latest closed bar 收盘已触发失效。",
        ...
        "4. **direction=neutral** 时仅顺 always_in：",
    ]
    if always_in == "AIL":
        ...
        lines.append("   - always_in 非 Always In → 无强信号则 wait，禁止边界双向刮头皮。")
...
def continuity_violation_reason(
    ...
    if direction == "neutral":
        if always_in == "AIL" and curr_sign < 0:
            return "direction=neutral 且 always_in=AIL：禁止做空方案"
        if always_in == "AIS" and curr_sign > 0:
            return "direction=neutral 且 always_in=AIS：禁止做多方案"
        if always_in is None and curr_sign != 0:
            return "direction=neutral 且 always_in 非 Always In：禁止无强信号的方向性方案"
