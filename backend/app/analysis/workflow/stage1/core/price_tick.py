...
def normalize_breakout_entry_price(
    ...
    *,
    ...
    raw_basis = decision.get("entry_basis_bar")
    if not isinstance(raw_basis, dict):
        return False
    try:
        bar = resolve_bar_ref(kline_frame, BarRef.model_validate(raw_basis))
    except ValueError:
        return False
...
def format_breakout_tick_hint(kline_frame: Any) -> str:
    ...
    return (
        ...
        f"推荐 `entry_price = 依据棒 high + {tick_s}`（禁止等于 high）；"
        f"做空时 `entry_price` 必须 **严格低于** low，推荐 `low - {tick_s}`。"
        f"`entry_rule` 必须写明依据棒开盘时间、周期、low/high 与实际价格，"
        f"勿重复 order_type/方向长句。"
        f"**程序会用 entry_basis_bar 对应棒的极点重算 entry_price，忽略你给的数值——"
        f"请确保 entry_basis_bar 身份与你实际引用的 K 线一致。**"
    )
