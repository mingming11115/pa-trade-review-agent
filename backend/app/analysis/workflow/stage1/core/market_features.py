...
from app.analysis.workflow.stage1.core.compat import infer_price_tick_from_frame
...
def render_simple_market_features(features: SimpleMarketFeatures, frame: KlineFrame) -> str:
    ...
    def label(seq: int) -> str:
        ref = refs[seq]
        return f"日内第{ref.day_index}根（{ref.bar_timestamp.strftime('%H:%M')} {ref.timeframe}）"

    def span(text: str) -> str:
        seqs = [int(value) for value in re.findall(r"K(\d+)", text)]
        return "至".join(label(seq) for seq in seqs) if seqs else text
    lines = [
    ...
    if features.swings:
        swing_text = ", ".join(
            f"{label(p.seq)}{'高' if p.kind == 'high' else '低'}{p.price}" for p in features.swings[:6]
        )
    ...
    if features.breakout_events:
        for ev in features.breakout_events[:4]:
            ...
            lines.append(
                f"- {event_zh} {kind_zh} **{ev.level_price}** @ {label(ev.trigger_seq)}"
                f"（{span(ev.bar_range)}）{ev.note}"
            )
    else:
    ...
    lines.append(
        ...
        + (f"，最近触发 {label(hc.last_bull_trigger_seq)}" if hc.last_bull_trigger_seq else "")
    )
    lines.append(
        f"- 空头计数：**{hc.bear_count}**（候选 {hc.bear_candidate.upper()}）"
        + (f"，最近触发 {label(hc.last_bear_trigger_seq)}" if hc.last_bear_trigger_seq else "")
    )
    lines.append(f"- 计数窗口：{span(hc.bar_range)}")
    ...
    if features.measured_moves:
        for mm in features.measured_moves[:4]:
            lines.append(
                ...
                f"（{span(mm.reference)}，{span(mm.bar_range)}）"
            )
    else:
