...
from app.analysis.workflow.stage1.core.coherence_checks import _CYCLE_BRANCH_ALIASES, _normalize_direction_branch
from app.analysis.workflow.stage1.core.decision_tree import load_decision_tree
from app.analysis.workflow.stage1.core.node_ids import canonicalize_node_id, node_order_key
...
def _coerce_bar_range(
    ...
    *,
    ...
    """Leave structured bar identities intact; legacy string repair is unsupported."""
    if item.get("skipped") and not str(item.get("answer", "")).strip():
        item["answer"] = "不适用"
...
def normalize_trace_item(
    ...
    *,
    ...
    nid = canonicalize_node_id(str(item.get("node_id", "")).strip().lstrip(""))
    item["node_id"] = nid
...
def _strip_ai_gate_14(gate_trace: list[Any]) -> None:
    ...
    for item in gate_trace:
        if isinstance(item, dict) and str(item.get("node_id", "")) == "prohibition_scan":
            if not seen_14:
                seen_14 = True
                kept.append(item)
            else:
...
def normalize_trace_list(
    ...
    *,
    ...
    # Reorder by semantic registry rank (structure → signal → risk → order → prohibition).
    def _chapter_rank(item: Any) -> tuple[int, str]:
        if not isinstance(item, dict):
            return (10_000, "")
        return node_order_key(str(item.get("node_id", "")))
    ...
    for item in trace:
        if isinstance(item, dict):
            if isinstance(item.get("bar_range"), dict) or (
                "bar_range" in item and item.get("bar_range") is None
            ):
                continue
...
def _sync_gate_12_branch_with_cycle(obj: dict[str, Any]) -> None:
    ...
    for item in gate:
        if not isinstance(item, dict) or str(item.get("node_id", "")).strip() != "cycle_identifiable":
            continue
        ans = str(item.get("answer", "")).strip()
...
def _repair_stage2_terminal(obj: dict[str, Any]) -> None:
    ...
    for item in trace:
        if not isinstance(item, dict) or str(item.get("node_id")) != "trader_equation":
            continue
        if item.get("answer") != "否":
            return
        old_nid = str(terminal.get("node_id", "") or "")
        if old_nid != "trader_equation":
...
def _sync_gate_23_answer_with_direction(obj: dict[str, Any]) -> None:
    ...
    for item in gate:
        if not isinstance(item, dict) or str(item.get("node_id", "")).strip() != "program_direction":
            continue
        if item.get("skipped"):
            return
...
def _repair_gate_result(obj: dict[str, Any]) -> None:
    ...
    - cycle_identifiable answer≠是 (cannot identify cycle)
    - not_extreme_chaos answer=是 (market is extremely chaotic)
    ...
    node_12_block = any(
        ...
        and str(item.get("node_id", "")) == "cycle_identifiable"
        and str(item.get("answer", "")).strip() != "是"
        for item in gate
    )
    node_13_block = any(
        isinstance(item, dict)
        and str(item.get("node_id", "")) == "not_extreme_chaos"
        and str(item.get("answer", "")).strip() == "是"
        for item in gate
    )
...
def _strip_program_reference_blocks(text: str) -> str:
    """Remove merged program-metric blocks from trace reason (momentum_enough cleanup)."""
...
def _repair_stage1_gate_trace(obj: dict[str, Any]) -> None:
    ...
    for item in gate:
        ...
        if nid in canonical_q:
            item["question"] = canonical_q[nid]
        if nid in ("not_extreme_chaos", "momentum_enough"):
            reason = str(item.get("reason", "") or "")
