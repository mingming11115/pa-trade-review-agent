...
def build_decision_stance_guidance(stance: str | None) -> str:
    ...
    common_rules = (
        "通用约束（各档都必须遵守）：\n"
        "- 仍必须完整输出 decision_trace，按 信号→风险→下单、prohibition_scan 走适用节点，不得伪造 trace。\n"
        "- 节点 trader_equation 须基于已拟定的 entry/stop/target 做数值判断；禁止无止损、无目标。\n"
        ...
        "trader_equation 方程可通过（期望为正），应输出限价单而非默认观望。\n"
        "- **signal_bar_quality / planned_limit 计划型限价**：宽通道/区间/通道靠边界、或顺势回撤到结构位时，"
        "无已收盘信号棒 → signal_bar_quality=否，**必须**评估 planned_limit；"
        "planned_limit=是 时可输出限价单，signal_bar.quality 可为 weak/invalid。\n"
        "- **direction=neutral 约束**：neutral 时 planned_limit **仅允许顺 always_in**（AIL→只做多侧，"
        "AIS→只做空侧；always_in=否→默认 wait）；禁止上下边界双向刮头皮。\n"
        "- **方案连续性**：若上一轮可执行方案**未失效**，默认 wait；"
        "同结构位 N 根内禁止反手（详见阶段二「方案连续性」块）。\n"
        "- **计划型限价最新棒对照**：只看 entry 相对最新已收盘棒 close 的方向；历史影线触及 entry 不算未来计划失效。\n"
        "- 完成 trader_equation 后必须在 decision 中填写 estimated_win_rate（0–100）与 estimated_win_rate_reasoning；"
        "order_type=不下单 时 estimated_win_rate 必须为 null。\n"
        "- **decision 与 trace/terminal 必须一致**："
        "decision_trace 中 trader_equation=否、terminal.outcome 为 wait/reject、或 prohibition_scan 判「是」时，"
        ...
        "- 触犯 prohibition_scan 硬性禁止项时，各档均须 order_type=不下单，"
    ...
    if normalized == "conservative":
        profile = (
            ...
            "- 信号链 入场：优先典型、清晰、收盘确认的一类信号；次优/模糊 setup 默认继续等待。\n"
            "- **planned_limit 计划型限价（边界例外）**：宽通道/区间/通道靠边界时，"
            "即使 direction=neutral、最新已收盘棒为 doji/弱棒/无信号棒，仍应先评估 planned_limit 边界限价三价；"
            "中部位置或无结构锚点时才默认 wait。\n"
            "- 风险节点：止损必须明确且不过大；trader_equation 交易者方程边际情况倾向判「否」。"
            "RR 须 ≥1.0 且方程通过。\n"
            "- prohibition_scan：从严扫描；有疑虑即不下单。\n"
        ...
        profile = (
            ...
            "**背景与周期优先于独立信号棒**。\n"
            "- planned_limit 计划型限价（**默认路径**）：当 cycle_position 为 broad_channel / "
            "trading_range / normal_channel / trending_tr，且 gate_result=proceed——"
            "**即使无强信号棒**，signal_bar_quality=否 后 **必须**写 planned_limit 并尝试背景限价三价。\n"
            "- planned_limit=是 时：signal_bar.bar_ref=null、quality=invalid/weak；entry_bar pending；"
            "**禁止**只在 watch_points 写方案而不填 decision 三价。\n"
            "- 信号链 入场：除典型信号外，若结构与阶段一 direction/cycle_position 一致，"
            "允许「次优但可执行」的二类 setup（须在 reason 中写明为何仍值得做）。\n"
            "- 宽通道/区间边界：优先限价单路径；weak/invalid 信号棒或无信号棒配合 "
            "tr_boundary/L1/H1 等 pattern 仍可下单，须在 信号链/reasoning 说明接受的瑕疵。\n"
            "- 风险节点：trader_equation 边际可通过时，若方程为正且结构清晰，可判「是」；"
            "须在 trade_confidence_reasoning 写明假设。\n"
            "- prohibition_scan：仅明确触犯才不下单；不要因「不够完美」单独放弃。\n"
        ...
        profile = (
            ...
            "- 信号链 入场：结构方向一致时，可接受更早、更不完美的入场触发；"
            "须在 reason 说明接受的瑕疵与失效条件。\n"
            "- 风险节点：trader_equation 在 entry/stop/target 已明确时，若方程略偏边际但方向与周期位置一致，"
            "可判「是」；须在 reasoning 强调风险克制。\n"
            "- prohibition_scan：仅硬性禁止项触发不下单；不要因为「理想目标位更远」而放弃可执行方案。\n"
        ...
        profile = (
            "【极度激进】= 强制产出交易；在 **trader_equation 可通过且未触犯 prohibition_scan** 时，"
            "必须给出具体进场方案，禁止因犹豫而输出「不下单」。\n"
            "- **拟下单路径**（仅当 trader_equation 判「是」且 terminal.outcome=trade 时生效）："
            "order_type 为「限价单」「突破单」「市价单」之一，"
            "order_direction 为「做多」或「做空」，"
            "entry/stop/target 为有效数值。\n"
            "- **不下单路径**（trader_equation=否、方程不达标、或 prohibition_scan=是）："
            "order_type=「不下单」，所有价格字段为 null；"
            "terminal.outcome=reject 或 wait；"
            "若主因是方程不通过，terminal.node_id 建议为 trader_equation。\n"
            "- **强制选定方向**（仅拟下单时）：综合阶段一、HTF 与最近 K 线，"
            "在多空之间选一个更优方向；neutral 时根据区间位置与近 3–5 根 K 线二选一。\n"
            "- 信号链：信号不完美也可判「是」，但须在 reason 写明接受的瑕疵。\n"
            "- trader_equation：突破单须先用依据 K 的极值±1跳动写出 entry，再写 stop/target，用这三价做检验；"
            "RR 须 ≥1.0 且胜率×回报>败率×风险，否则**必须**判「否」并走不下单路径，"
            "不得保留具体下单价格；trace 中的风险/回报数字须与 decision 三价一致。\n"
            "- 突破单不可行时，优先评估限价单结构位方案，方程通过即可下单。\n"
            "- 可在 trader_equation 通过时合理估算胜率（约 45–55%），"
            "边际通过须在 trade_confidence_reasoning 说明。\n"
            "- trade_confidence 可低至 25–40，但仅适用于 **trader_equation=是** 的下单方案。\n"
        )
