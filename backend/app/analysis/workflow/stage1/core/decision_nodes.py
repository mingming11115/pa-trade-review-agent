...
- DecisionNodeEngine: deterministic judge for data_sufficiency/program_direction/always_in/9/11 nodes
...
from app.analysis.workflow.stage1.core.node_ids import is_prohibition_node
...
from app.analysis.workflow.stage1.core.bar_identity import BarRef, resolve_bar_ref
...
BAR_COUNT_THRESHOLD: int = 20           # data_sufficiency data sufficiency threshold

DIRECTION_WINDOW: int = 8               # program_direction direction voting window (short) — 缩小到8根，重点捕捉近期结构突变
DIRECTION_WINDOW_MED: int = 20         # program_direction medium window — 仅作背景参考，强短窗口时不扣分
DIRECTION_STRONG_SHORT_SCORE: int = 4   # |score|≥此值时忽略中窗口冲突（新趋势优先于旧背景）

ALWAYS_IN_NEAR_WINDOW: int = 8          # always_in 近端主判（Brooks：惯性=刚刚在做的事）
ALWAYS_IN_WINDOW: int = 20             # always_in 背景参考窗口（不否决近端结论）
ALWAYS_IN_NEAR_SAME_SIDE_RATIO: float = 0.65  # 近端加权同侧占比（8根窗口略低于20根阈值）

ALWAYS_IN_SAME_SIDE_RATIO: float = 0.7  # always_in same-side ratio threshold
ALWAYS_IN_PULLBACK_ATR_RATIO: float = 1.5  # always_in max pullback depth (×ATR) for AIL/AIS

SIGNAL_BAR_LONG_ATR_RATIO: float = 2.0  # signal_not_overlong overlong threshold

EMA_SLOPE_LOOKBACK: int = 10            # EMA slope lookback bars

# ── program_direction direction vote thresholds ────────────────────────────────────────────
...
LOCKED_NODES: frozenset[str] = frozenset({"data_sufficiency", "signal_bar_closed"})

OVERRIDABLE_NODES: frozenset[str] = frozenset({

    "not_extreme_chaos", "program_direction", "always_in", "momentum_enough", "signal_direction_ok", "signal_not_overlong", "order_market", "order_breakout", "order_limit", "order_breakout_entry",
...
AI_PRIMARY_NODES: frozenset[str] = frozenset({"not_extreme_chaos", "momentum_enough"})
...
# not_extreme_chaos extreme chaos thresholds
...
# momentum_enough momentum strength thresholds
...
SAFETY_GATE_NODES: frozenset[str] = frozenset({"data_sufficiency", "trader_equation", "prohibition_scan"})
...
@dataclass(frozen=True)
class BarSpan:
    """Internal frame positions; converted to stable BarRange at the boundary."""
...
def _node_label(node_id: str) -> str:
    ...
    try:
        ...
        from app.analysis.workflow.stage1.core.decision_tree import node_label as _nl
...
def build_program_trace_node(fill: NodeFill, *, tree: Any = None, frame: Any = None) -> dict[str, Any]:
    ...
    bar_range: Any = None
    reason = fill.reason
    if frame is not None and fill.bar_range is not None:
        from app.analysis.workflow.stage1.core.bar_identity import BarRange, assign_bar_refs

        refs = {bar.seq: ref for bar, ref in zip(
            frame.bars,
            assign_bar_refs(frame.bars, symbol=frame.symbol, timeframe=frame.timeframe),
        )}
        oldest_seq = fill.bar_range.oldest_seq
        newest_seq = fill.bar_range.newest_seq
        if oldest_seq in refs and newest_seq in refs:
            bar_range = BarRange(start=refs[oldest_seq], end=refs[newest_seq]).model_dump(mode="json")
        def replace_relative(match: re.Match[str]) -> str:
            ref = refs.get(int(match.group(1)))
            if ref is None:
                return "未找到的棒"
            opened = ref.bar_timestamp.strftime("%H:%M")
            return f"日内第{ref.day_index}根（{opened} {ref.timeframe}）"
        reason = re.sub(r"K(\d+)", replace_relative, reason)
...
def judge_data_sufficiency(frame: Any) -> NodeFill:
    ...
    """Fill data_sufficiency=是 (data already sufficient, PreflightDataGate already passed)."""
...
def judge_market_chaos(frame: Any) -> NodeFill:
    """Judge not_extreme_chaos: is the market in extreme chaos (extreme_tr)?
    ...
    # ── C3: No directional conviction — reuse direction score from program_direction ──────
    # We compute a simplified 2-signal score here to avoid calling judge_direction
    # twice; the full 5-signal program_direction result will still be injected separately.
    ...
    if chaos_score == 3:
        warning = (
            ...
            "如AI综合判断确认极端混乱，可在 node_overrides 中提交 not_extreme_chaos=是 覆盖。"
        )
    elif chaos_score == 2:
        warning = (
            f" ⚠️ 两项混乱指标触发（{chaos_score}/3），"
            "AI可结合整体K线结构判断是否构成极端混乱；若是，可提交 node_overrides not_extreme_chaos=是。"
        )
    else:
        warning = ""
...
def judge_direction(frame: Any) -> tuple[str, NodeFill]:
    """
    五信号投票机制，用于判定行情方向并填充program_direction节点。
    信号（每个信号对总得分贡献‑1、0或+1）：
      S1：EMA斜率（回看10根K线，ATR死区滤波）
      S2：收盘价重心‑短周期窗口（20根K线，对比近半区间与远半区间）
      S3：摆动结构：高点抬高+低点抬高(HH+HL) 对比 低点降低+高点降低(LL+LH)（20根K线窗口内采用2根K线枢轴点检测）
      S4：趋势K线占比（20根K线内看涨趋势K线与看跌趋势K线计数比值）
      S5：K线重叠率（重叠度低→趋势行情；重叠度高→震荡/无明确方向）

    当中周期窗口确认信号（50根K线收盘价重心）与短窗口结论相悖时，总得分绝对值减1。

    阈值由±2上调至±3，使信号在宽通道与震荡区间中表现更稳定，与《二元决策.txt》的program_direction章节保持一致。

    返回值：(方向, NodeFill)；方向取值集合：{看涨，看跌，中性}。
    """
    ...
    # program_direction "多头趋势棒占优" / "空头趋势棒占优"
    s4 = 0
    s4_desc = "趋势棒占比:0"
    try:
        bull_tb, bear_tb = _count_trend_bars(bars, W)
    ...
    # momentum_enough "K线重叠少→趋势强" / "K线重叠多→区间/无方向"
...
def _max_pullback_atr(bars: Any, N: int, ema20: tuple, atr14: tuple) -> float | None:
    ...
    Used by judge_always_in to verify always_in "回撤浅" condition.
    """
    try:
...
def judge_always_in(frame: Any) -> NodeFill:
    """Judge Always In state (always_in) with dual-window Brooks alignment.
...
def judge_momentum_strength(frame: Any, direction: str = "neutral") -> NodeFill:
    """Judge momentum_enough: is current momentum strong enough to support trend-following?
    ...
    Scoring:
      ...
      strong_count == 0 → answer=否  (weak; NOT gate=wait per momentum_enough rules)

    Since momentum_enough is AI_PRIMARY, if AI already wrote this node the program result
    ...
    if strong_count >= 2:
        ...
        branch = None
        conclusion = (
            "惯性偏弱，不宜趋势跟踪；但momentum_enough否不触发gate=wait，"
            "继续进入策略分支等待合适入场时机。"
        )
...
def _get_signal_seq(out: dict[str, Any], frame: Any) -> int:
    """Resolve the structured signal identity to the frame's internal sequence."""
    try:
        signal_bar = out.get("bar_analysis", {}).get("signal_bar", {})
        ref = BarRef.model_validate(signal_bar.get("bar_ref"))
        return int(resolve_bar_ref(frame, ref).seq)
    except (AttributeError, TypeError, ValueError):
        return 1

def has_background_limit_path(out: dict[str, Any]) -> bool:
    """True when decision_trace records planned_limit=是 (background-driven limit path)."""
    ...
    for item in trace:
        if not isinstance(item, dict):
            continue
        if str(item.get("node_id", "")).strip() != "planned_limit":
            continue
...
def is_planned_limit_order(out: dict[str, Any]) -> bool:
    ...
    pending = (
        ...
        or entry_bar.get("bar_ref") is None
    ...
    if signal_bar.get("bar_ref") is None and quality in ("invalid", "weak"):
        return True
...
def judge_signal_bar_closed(sig: int, frame: Any) -> NodeFill:
    ...
    """signal_bar_closed: signal bar is always closed (all bars in KlineFrame are closed)."""

    return NodeFill(

        node_id="signal_bar_closed",

        answer="是",

        reason=f"K{sig}为已收盘K线（KlineFrame内所有K线均已收盘），可作为信号棒。",

        bar_range=BarSpan(sig, sig),

    )

# signal_direction_ok direction consistency sets
...
def judge_signal_bar_direction(
    ...
    """signal_direction_ok: check signal bar direction consistency with order direction.
    ...
    if not order_direction or order_direction not in ("做多", "做空"):
        return NodeFill(
            node_id="signal_direction_ok",
            answer="不适用",
            reason="无交易计划方向（order_direction缺失），signal_direction_ok不适用。",
            bar_range=None,
        )
...
def judge_signal_bar_length(sig: int, features: dict[int, Any]) -> NodeFill:
    ...
    """signal_not_overlong: check if signal bar is overlong (range_atr_ratio > 2.0)."""
...
def judge_follow_through(sig: int, features: dict[int, Any]) -> NodeFill:
    ...
    """follow_through: follow_through_1_2 mapping."""
    ...
    if sig > 1:
        ...
        bar_range = BarSpan(sig)

    else:

        bar_range = BarSpan(1, 1)

    return NodeFill(

        node_id="follow_through",
...
def route_order_method(
    ...
    # Check safety gates: trader_equation=否 or prohibition_scan violation
    ...
    if _trace_answer(decision_trace, "trader_equation") == "否":
    ...
    def _sec14_violated(trace: list) -> bool:
        ...
        for item in trace:
            ...
            if not is_prohibition_node(nid):
    ...
    # Preserve model's explicit limit/market choice when trader_equation already passed.
    if (
        model_order_type == "限价单"
        and _trace_answer(decision_trace, "trader_equation") == "是"
    ...
    # is the textbook entry (3.4 SPS / spike_path path-A).  Forcing 市价单 on a pending
    ...
    if candidate == "突破单":
        ...
        if not has_basis:
            ...
            # No breakout anchor → try limit at structural level (if trader_equation already passed).
    ...
    _METHOD_NODE: dict[str, tuple[str, str]] = {
        ...
        "spike":         ("order_market", "市价单"),

        "micro_channel": ("order_breakout", "突破单"),

        "tight_channel": ("order_breakout", "突破单"),

        "normal_channel":("order_breakout", "突破单"),

        "broad_channel": ("order_breakout", "限价单"),

        "trading_range": ("order_limit", "限价单"),

        "trending_tr":   ("order_breakout", "突破单"),
    ...
    all_nodes = ["order_market", "order_breakout", "order_limit", "order_breakout_entry"]

    final_idx = all_nodes.index(final_node_id) if final_node_id in all_nodes else -1

    _node_reasons: dict[str, str] = {

        "order_market": "趋势/尖峰阶段，价格快速移动，适合市价单立即入场。",

        "order_breakout": "通道结构，等待突破确认，使用突破单。",

        "order_limit": "交易区间，在区间边界附近使用限价单。",

        "order_breakout_entry": "宽通道/特殊情况，使用限价单。",
        ...
        if nid == final_node_id:
            ...
            if cycle == "spike" and candidate == "突破单" and spike_stage_label in ("ending", "pullback", "channel"):
                ...
                reason = (
                    ...
                    f"trader_equation 已通过 → 改用限价单在结构位挂单（回撤/反弹到位入场）。"
                    + reason
                )
            else:
...
def _conservativeness_rank(node_id: str, answer: str) -> int:
    ...
    if nid == "trader_equation":

        return 5 if ans == "否" else 3

    if nid == "prohibition_scan":

        return 5 if ans == "是" else 3

    # order_type dimension (11 nodes)

    if nid in ("order_market", "order_breakout", "order_limit", "order_breakout_entry"):
...
def _node_id_sort_key(node_id: str) -> tuple[int, int, str]:
    ...
    Converts 'data_sufficiency' -> (1, 1, 'data_sufficiency'), 'program_direction' -> (2, 3, 'program_direction') so that merged
...
def merge_program_nodes(
    ...
    PROGRAM-AUTHORITATIVE (default):
      Program result replaces the AI node entirely.  Used for data_sufficiency, program_direction, always_in
      where the program has definitive computed data.

    AI-PRIMARY (AI_PRIMARY_NODES — not_extreme_chaos and momentum_enough):
    ...
    for i, item in enumerate(result):
        ...
        if nid in AI_PRIMARY_NODES:
            ...
            if nid in AI_PRIMARY_SUPPLEMENT_NODES:
                # AI-primary + program supplement in reason (not_extreme_chaos only)
            ...
            # momentum_enough: keep AI node as-is; program metrics are not appended to reason.
...
def merge_program_nodes_head(
    ...
    not_extreme_chaos and momentum_enough preserve the AI version without appending program data to reason.
    """
...
def apply_overrides(
    ...
    *,
    ...
    6. program_direction direction consistency check

    7. valid override → accept, write trace

    """

    from app.analysis.workflow.stage1.core.decision_tree import TRACE_ANSWERS
    ...
    for ov in node_overrides:
        ...
        # Rule 6: program_direction direction consistency

        if node_id == "program_direction":
            ...
            if not valid:
                ...
                logger.debug(
                    ...
                    "apply_overrides: rejecting program_direction override - "
        ...
        if node_id in OVERRIDABLE_NODES:
            ...
            if idx is not None:
                ...
                if node_id in ("order_market", "order_breakout", "order_limit", "order_breakout_entry"):

                    _sync_order_type_from_11_override(out, result[idx], ov)

                # always_in override: sync bar_analysis.always_in so the field stays
                # consistent with the final (possibly AI-overridden) always_in branch.
...
def _validate_dir_override(answer: str, branch: str) -> bool:
    ...
    """Validate program_direction answer/branch consistency."""
...
def _sync_always_in_from_24_override(
    ...
    """After always_in override accepted, sync bar_analysis.always_in to match the
    AI-overridden branch.  Without this sync, bar_analysis.always_in keeps the
    program's original value while the gate_trace always_in node shows the overridden
...
def _sync_order_type_from_11_override(
    ...
    # Mirror judge_section11 breakout_fallback_to_limit: order_breakout defaults to 突破单,

    # but without basis fields the schema rejects null entry_basis_*. Preserve an

    # explicit 限价单/市价单 plan (e.g. planned_limit planned limit) instead of forcing 突破单.
...
class DecisionNodeEngine:
    ...
    def apply_stage1(out: dict[str, Any], frame: Any) -> None:
        ...
        """In-place modify stage1 JSON: fill data_sufficiency/program_direction/always_in, apply overrides, update direction."""

        # Ensure gate_trace exists

        out.setdefault("gate_trace", [])

        # Step 1: DataSufficiencyJudge → data_sufficiency=是

        fill_11 = judge_data_sufficiency(frame)

        # Step 1b: MarketChaosJudge → not_extreme_chaos
        ...
        # Step 2: DirectionJudge → program_direction + direction field

        direction, fill_23 = judge_direction(frame)

        out["direction"] = direction

        # Step 3: AlwaysInJudge → always_in

        fill_24 = judge_always_in(frame)

        # Step 3b: MomentumStrengthJudge → momentum_enough
        # Assesses trend-bar dominance, overlap, pullback depth.
        # Overridable; per momentum_enough rules, answer=否 does NOT trigger gate=wait.

        fill_25 = judge_momentum_strength(frame, direction=direction)

        # Convert NodeFill → trace dicts

        node_11 = build_program_trace_node(fill_11, frame=frame)

        node_13 = build_program_trace_node(fill_13, frame=frame)

        node_23 = build_program_trace_node(fill_23, frame=frame)

        node_24 = build_program_trace_node(fill_24, frame=frame)

        node_25 = build_program_trace_node(fill_25, frame=frame)

        # momentum_enough is a NON-BLOCKING gate node: any answer (是/中性/否) still results in
        ...
        # Step 6: Sync bar_analysis.always_in from the program-determined always_in node.
        ...
        # where the program fills always_in directly and bar_analysis.always_in must match.
        node_24_final = next(
            (n for n in final_nodes if isinstance(n, dict) and str(n.get("node_id", "")) == "always_in"),
            None,
        )
        ...
        try:
            from app.analysis.workflow.stage1.core.trend_context import build_trend_context
    ...
    def apply_stage2(
        ...
        """In-place modify stage2 JSON: fill signal_bar_closed/signal_direction_ok/signal_not_overlong/follow_through/11, apply overrides."""
        ...
        sig = _get_signal_seq(out, frame)

        # Check signal_bar_quality answer: if AI said no valid signal bar, skip signal_bar_closed-9.5
        ...
        # Both map to skip signal_bar_closed-9.5 — unless this is a planned limit order.
        _dt = out["decision_trace"]
        _node_90 = next(
            (x for x in _dt if isinstance(x, dict) and str(x.get("node_id", "")) == "signal_bar_quality"),
            None,
        )
        ...
        # Step 1: SignalBarJudge → signal_bar_closed, signal_direction_ok, signal_not_overlong
        ...
        # Step 2: FollowThroughJudge → follow_through
        ...
        node_91 = build_program_trace_node(fill_91, frame=frame)

        node_92 = build_program_trace_node(fill_92, frame=frame)

        if fill_92.answer == "不适用":

            node_92["skipped"] = True

        node_93 = build_program_trace_node(fill_93, frame=frame)

        node_95 = build_program_trace_node(fill_95, frame=frame)

        # When signal_bar_quality=否 (no valid signal bar), mark signal_bar_closed-9.5 as skipped so they
        # don't appear as contradictory program-filled nodes in the trace.
        if not _section9_has_signal:
            _skip_reason = "signal_bar_quality=否（无有效信号棒），signal_bar_closed-9.5不适用，程序跳过。"
            ...
            _no_signal_bar = (
                not isinstance(_signal_bar, dict) or not _signal_bar.get("bar_ref")
            )
            if _no_signal_bar or has_background_limit_path(out):
                _skip_reason = (
                    "计划型限价单（planned_limit 或 signal_bar_quality 背景路径），尚无已收盘信号棒，"
                    "signal_bar_closed-9.3不适用。"
                )
        ...
        sec11_nodes = [build_program_trace_node(f, frame=frame) for f in sec11_fills]
