...
from app.analysis.workflow.stage1.core.retry_policy import StageName, extract_feedback_targets
from app.analysis.workflow.stage1.core.compat import format_validation_errors
...
_FORBIDDEN_STAGE1 = (
    ...
    "cycle_position，并用 node_overrides program_direction 说明依据）",
    "bar_by_bar_summary[].bar_type（必须服从程序几何表）",
    "程序锁定节点 data_sufficiency",
)
...
def _geometry_excerpt(frame: Any, limit: int = 8) -> str:
    try:
        ...
        for f in feats:
            lines.append(f"  窗口位置{f.seq}: {f.bar_type}")
        return "\n".join(lines)
...
def build_retry_feedback(
    ...
    *,
    ...
    if stage == "stage1" and (
        ...
        lines.append(
            "- background_near_term_coherent：`answer` 只能用 **是/否/中性/等待/不适用**；"
    ...
    if stage == "stage2" and any("decision.reasoning" in inv for inv in invalid):
        ...
        lines.append(
            ...
            "细节放在 decision_trace trader_equation 或 key_factors。"
        )
