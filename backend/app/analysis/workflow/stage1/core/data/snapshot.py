...
from app.analysis.workflow.stage1.core.data.base import IndicatorBundle, KlineBar, KlineFrame, normalize_kline_bar
...
def _newest_closed_slice(
    ...
    *,
    ...
    if not bars_raw or n < 1:
        return None
    forming = bool(bars_raw and not bars_raw[0].closed)
...
def compute_indicators(bars: list[KlineBar]) -> IndicatorBundle:
    ...
    """
    from app.analysis.workflow.stage1.core.indicators.ema import ema_full
    from app.analysis.workflow.stage1.core.indicators.atr import atr_full
...
def build_live_frame(
    ...
    *,
    ...
    """
    has_forming = bool(bars_raw and not bars_raw[0].closed)
    if has_forming:
    ...
    return KlineFrame(
        ...
        snapshot_ts_local_ms=int(time() * 1000),
    )
