...
# Tokens in 信号链 / decision reasoning that justify trading on weak|invalid signal_bar.
...
class JsonValidator:
    ...
    def __init__(self, validation: Any = None) -> None:
        from app.analysis.workflow.stage1.core.prompts.schemas import STAGE1_SCHEMA, STAGE2_SCHEMA
        from app.analysis.workflow.stage1.core.compat import ValidationSettings
    ...
    def normalize_parsed(
        ...
        *,
        ...
        if stage == "stage1":
            from app.analysis.workflow.stage1.core.stage1_normalizer import normalize_stage1
        ...
        from app.analysis.workflow.stage1.core.stage2_normalizer import normalize_stage2
    ...
    def validate(
        ...
        *,
        ...
        if not stripped.startswith("{") and not stripped.startswith("["):
            from app.analysis.workflow.stage1.core.provider_errors import (
        ...
        if stage == "stage1":
            from app.analysis.workflow.stage1.core.coherence_checks import auto_fix_bar_by_bar_types
            ...
            if getattr(self._validation, "stage1_coherence_checks", False):
                from app.analysis.workflow.stage1.core.decision_tree import validate_gate_result_consistency
                from app.analysis.workflow.stage1.core.coherence_checks import (
            ...
            if getattr(self._validation, "trace_semantic_checks", False):
                from app.analysis.workflow.stage1.core.trace_semantic_checks import validate_trace_semantics
        ...
        if stage == "stage2":
            ...
            if getattr(self._validation, "stage2_coherence_checks", False):
                from app.analysis.workflow.stage1.core.decision_tree import validate_stage2_trace_consistency
                from app.analysis.workflow.stage1.core.coherence_checks import validate_stage2_coherence
            ...
            if getattr(self._validation, "trace_semantic_checks", False):
                from app.analysis.workflow.stage1.core.trace_semantic_checks import (
    ...
    def _check_breakout_order_basis(obj: dict) -> dict | None:
        ...
        if not decision.get("entry_basis_bar"):
            ...
            allowed["decision.entry_basis_bar"] = ["structured bar identity"]
    ...
    def _check_breakout_price_extreme(obj: dict, kline_frame: Any = None) -> list[str]:
        ...
        basis = _parse_bar_ref(decision.get("entry_basis_bar"))
        if basis is None:
            return []
        try:
            bar = resolve_bar_ref(kline_frame, basis)
        except ValueError:
            return ["entry_basis_bar not found in current K-line frame"]
    ...
    def _check_next_cycle_prediction(obj: dict) -> list[str]:
        ...
        """
        from app.analysis.workflow.stage1.core.cycle_enums import CYCLE_ENUM, CYCLE_ORDER
    ...
    def _check_signal_chain(
        ...
        *,
        ...
        """Require order decisions to ground 信号链 in signal/entry/follow-through facts."""
        decision = obj.get("decision", {})
        if not isinstance(decision, dict):
            return []
        ...
        signal_ref = _parse_bar_ref(signal_bar.get("bar_ref"))
        entry_ref = _parse_bar_ref(entry_bar.get("bar_ref"))
        ...
        planned_without_signal = (
            ...
            and signal_bar.get("bar_ref") is None
        )
        ...
        planned_limit_weak = (
            ...
            and quality == "weak"
            and (
                signal_bar.get("bar_ref") is None
                or pattern in ("", "none", *_planned_limit_boundary_patterns)
            )
        )
        # planned_limit planned limit: invalid + boundary pattern + no closed signal bar.
        ...
        if signal_ref is None and not planned_entry:
            errors.append("bar_analysis.signal_bar.bar_ref must be a structured bar identity")
        if entry_ref is None and not pending_entry:
            errors.append("bar_analysis.entry_bar.bar_ref must be a structured bar identity")
        if pending_entry and decision.get("order_type") == "市价单":
            errors.append("market order requires a concrete entry_bar.bar_ref")
        if signal_ref is not None and entry_ref is not None and signal_ref.bar_timestamp >= entry_ref.bar_timestamp:
            errors.append("signal_bar must be older than entry_bar")
        if kline_frame is not None:
            for label, ref in (("signal_bar", signal_ref), ("entry_bar", entry_ref)):
                if ref is not None:
                    try:
                        resolve_bar_ref(kline_frame, ref)
                    except ValueError:
                        errors.append(f"bar_analysis.{label}.bar_ref not found in current K-line frame")
        ...
        if (
            ...
            if not any(token in reasons for token in _EXPLICIT_S9_TRADABLE_TOKENS):
                errors.append(
                    "weak/invalid signal_bar requires explicit 信号链 reasoning for why the setup remains tradable"
                )
...
def _parse_bar_ref(value: object) -> BarRef | None:
    try:
        return BarRef.model_validate(value)
    except (TypeError, ValueError):
        return None
